...
- All three interfaces fall back to `app-11.7.5.205_rom-36` when the requested registry is missing and `WIDGET_SERVICE_ENABLE_DEFAULT_CAPABILITY_REGISTRY_FALLBACK=true`. The generation interface records the actual fallback version in artifact metadata.
