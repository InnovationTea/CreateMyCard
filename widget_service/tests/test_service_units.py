...
def test_design_compact_prompt_carries_extrainfo_without_extending_task_spec(
    ...
    facts = [
        "演出时间为 2026 年 9 月 22 日 19:30（北京时间），不建议迟到。",
        "地点是上海大剧院，地址为人民大道 300 号。",
        "票价为 180 元，含服务费。",
    ]
    ...
    assert "STATIC_FACTS_TO_DISPLAY" in prompt[0]["content"]
    assert "STATIC_CONTENT_PAYLOAD=" in prompt[0]["content"]
    assert "ITEM 1:" in prompt[0]["content"]
    assert "ITEM 2:" in prompt[0]["content"]
    assert "ITEM 3:" in prompt[0]["content"]
    assert "必须完整读取" in prompt[0]["content"]
    assert "展示层可根据卡片尺寸和可读性进行摘要、压缩、合并、换行或改写" in prompt[0]["content"]
    assert "不得捏造、改变事实含义或丢失" in prompt[0]["content"]
    assert all(fact in prompt[0]["content"] for fact in facts)
    assert "不属于 TaskSpec" in prompt[0]["content"]
    assert prompt[0]["content"].index(facts[0]) < prompt[0]["content"].index(facts[1])
    ...
    assert "STATIC_FACTS_TO_DISPLAY" in generic_prompt[0]["content"]
    assert (
        "展示层可根据卡片尺寸和可读性进行摘要、压缩、合并、换行或改写"
        in generic_prompt[0]["content"]
    )


def test_extrainfo_route_rules_override_minimal_field_selection():
    task_spec = TaskSpecBuilder().build(
        user_query="制作静态演出信息卡片",
        size="2x2",
        effective_bindings=[],
        effective_data_capabilities=[],
        event_candidates=[],
    ...
    instruction = PromptBuilder._visual_route_instruction(
        task_spec,
        has_static_facts=True,
    )
    layout_lock = PromptBuilder._layout_route_lock(
        task_spec,
        PromptBuilder._layout_scope(task_spec),
        has_static_facts=True,
    )

    assert "STATIC_FACTS_TO_DISPLAY 共同决定内容范围" in instruction
    assert "相关静态事实必须进入可见内容" in instruction
    assert "优先于装饰、弱字段和非必要候选" in layout_lock


def test_extrainfo_repair_prompt_rechecks_each_static_fact():
    task_spec = TaskSpecBuilder().build(
        user_query="制作静态演出信息卡片",
        size="2x2",
        effective_bindings=[],
    ...
    facts = ["演出时间为今晚 19:30（北京时间）。"]
    initial_prompt = PromptBuilder().build(task_spec, extrainfo=facts)

    repair_prompt = PromptBuilder().build_repair(
        initial_prompt,
        "invalid-source-dsl",
        [{"stage": "validation", "code": "MISSING_TEXT", "message": "missing fact"}],
    )

    assert facts[0] in repair_prompt[0]["content"]
    assert "逐条核对每个与" in repair_prompt[0]["content"]
    assert "允许为尺寸和可读性摘要、合并或改写" in repair_prompt[0]["content"]
    assert "不得捏造或改变事实含义" in repair_prompt[0]["content"]
