...
2x4 数据块按语义对象和信息时间范围划分，不能只数 `/data` 一级根；同一 `healthSport` 根内的 `daily*` 日汇总与 `exercise*` 单次运动记录固定算两个数据块。最终恰好两个业务数据块时必须参考 V09：root 必须是 Row，并直接使用左右两个 `144×136vp` 大内容背板，不生成公共标题、公共内容区或公共按钮行，每个业务的数据和按钮只放在所属背板内。禁止复用 2x2 S4，禁止 `root Stack -> content Column`，禁止生成上下两个或更多 `296×48-64vp` 全宽内容背板或任何其它上下布局。
