...
- 每侧按 2x2 的层级组织；需要突出量化主值时按「对象名 + 主读数 + 必要辅助信息 + 可选底部按钮 120×36vp」排列；有必要对象名时先说明对象，再突出主值；没有突出数字时仍可按「业务标题 12fp/400 + 主内容 + 可选底部按钮」排列。有按钮时主内容使用剩余高度，每侧至多保留两行最小充分信息。业务标题只是背板内分区标题，不使用 CardHeader。
