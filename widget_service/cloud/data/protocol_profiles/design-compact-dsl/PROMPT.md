...
5. `layoutWeight`、`flexShrink`、`clip` 和分布式对齐都不能抵消已经算出的固定高度。`H_required > 126` 时必须删除弱区域、合并标题、降低区域固定高度或改选更简单骨架。

以下两种结构无论截图是否暂时可见都必须判定失败：

- `20 + 59 + 59 + 8 × 2 = 154 > 126`：标题行与 S4 纵堆双 `59vp` 区域不可共存；删除总标题，保留 `59 + 8 + 59 = 126`。
...
选择骨架前必须先按最终展示数据识别独立业务对象：同一语义实体下的名称、电量、状态等多个字段始终只算 1 个对象，`eventCandidates`、动作数量和组件数量均不得增加对象数；不同业务实体或同类业务中的不同对象实例（如两座城市）才分别计数。只有恰好 2 个独立对象时才允许 S4，且必须完整生成上下两个 `126×59vp` 分区；只有 1 个对象时必须选择 S1/S2/S3，绝对禁止生成单个 `126×59vp` 内容背板或其它残缺 S4。单对象加两个明确动作必须选择 S3，不得把该对象的字段拆成两个 S4 分区。
...
- region：只允许上下纵堆，固定为 `root -> [zone_top, zone_bottom]`，root `itemMargin:8`；两 zone 必须同时存在且均为 `126×59vp`、borderRadius 16、白色 80% 背板，且 `59 + 8 + 59 = 126`，禁止只生成其中一个 zone。不得增加 title/header/footer/action_area，也禁止左右双业务布局。每个 zone 必须完整遵守 8.3 节小内容蒙版规则，文字在左、图标或环图在右。
...
- region：无卡级总标题；`root Row` 以 `width/height:matchParent` 填满 `300×150vp` 画布，padding 12、itemMargin 8，直接包含左右两个 `134×126vp` 同级背板。两背板均为 Column、padding 12、borderRadius 16、白色 80% 背板，内部可用空间均为 `110×102vp`。这是 2x4 双业务唯一合法的一级布局，不得套用 2x2 S4 的上下两个 `126×59vp` 结构，也不得放大为上下两个 `276×59vp` 背板。
...
4. **骨架与预算**：是否保留每个并列对象的名称和有歧义指标的标签；必要文字是否使用主文字色而非 60%；普通 S2 是否主信息顶对齐、描述紧跟、动作沉底；是否只使用一个固定骨架；单业务双按钮是否为 S3 的 40vp 信息组加 78vp 动作组，主信息是否合并为 14fp 且没有独立标题和大数字内容区；是否按最终展示对象正确统计数据块，2x4 的 1/2/3/4 个数据块是否分别路由到 W1-W7/W9/W10/W8，2x2 恰好两个数据块是否强制使用完整双分区 S4，单数据块是否未生成单个 `126×59vp` 内容背板或残缺 S4，且没有把任一数据块降级为另一数据块的辅助信息；2x2 S4 两个背板内的文字、可染色图标、Progress 和 Divider 是否统一使用同一主内容色 RGB；所有小内容蒙版是否只有两行、左文字右 visual，1-2 项如有右侧图标，是否语义准确且状态安全，3 项时是否去掉 visual 并以 ` | ` 合并辅助数据，超过 3 项是否删除低优先级项，图标或环外框是否距蒙版右边 12vp；CardHeader 是否只用于 2x2 S2 或 2x4 单数据块的独立卡级标题、每卡最多一个且位于规定前景 Column 首项，W8/W9/W10 是否没有卡级标题、CardHeader、公共标题行或标题空槽，无标题布局是否未留空槽，S1/S3/S4 是否未加 CardHeader 或右上角辅助图标；root 宽高是否为 `"matchParent"`、padding 12、圆角 20、clip true；root Column 是否使用 `alignItems:"center"`，root Row 的固定参考宽度直接内容是否使用 `justifyContent:"center"`，且没有把 root 的整组居中误写成内部文字全部居中；是否已按 3.1 节逐项写出 root 直接子节点的最小高度、margin 和 `itemMargin` 并确认 `H_required <= 126vp`；分布式对齐是否只分配扣除最小间距后的非负剩余空间；所有 Row/Column 两轴预算是否非负，动态文字 Row 是否保留余量，点击热区是否至少 24vp。
