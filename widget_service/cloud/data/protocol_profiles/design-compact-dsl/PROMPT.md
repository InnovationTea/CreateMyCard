...
- CardHeader 封装独立卡片标题和可选右上角辅助图标，只输出一行，不带 children、动作或布局样式；2x2 仅用于 S2 有独立标题的布局，S1、S3、S4 不使用；2x4 仅允许单数据块布局按骨架使用，2-4 个数据块的 W8/W9/W10 不使用，无标题布局和作为正文上下文的 kicker 不使用，也不预留空标题位。
...
- 小内容蒙版使用图标时固定为 `Row -> [text_column, visual]`，visual 在右、垂直居中，右边缘距蒙版右边 12vp，与文字列间距 8vp。独立 Image 固定 `20×20vp`：2x2 文字列宽 84vp，2x4 文字列宽 92vp。仅有 1-2 项信息且有进度语义时，visual 可以是右侧环图，按环外框右边缘距蒙版右边 12vp：2x2 S4 使用 `44×44vp` 环和 60vp 文字列，2x4 W8 使用 `40×40vp` 环和 72vp 文字列；中心图标保持 20×20vp。3 项时必须删除包括 Image、Progress 和环中心内容在内的整个 visual；无合法图标且无合法环图时同样删除 visual，文字列占满蒙版内部宽度。
...
- region：`value_group`（hero 数值 + 单位）必须作为主内容第一行，居中或沉底；可选居中说明放在数值下方。S1 的说明属于主内容组，不使用 CardHeader，不额外生成右上角辅助图标。
...
- 用于：恰好两个独立业务对象的同级展示；同一对象的多个字段或多个动作不算双业务。满足时优先于其他 2x4 骨架，除非用户明确指定其他结构，或每侧的 `mustKeep` 无法在下述容量内完整展示。
...
- action：每侧至多一个属于该业务的可见 Button 或图文按钮 Row，事件只绑定该按钮；root、背板和非按钮内容均不得绑定 `onClick`。只有一侧有动作时，另一侧不生成按钮或空槽；不得生成卡级 action_area。
...
- 需要突出量化主值时，数字及其紧邻单位必须位于所属业务主内容区第一行，其他需要突出的文字放在数字下方。CardHeader 和会议日期上下文不计入主内容行；环形 Progress 视为主内容第一项；S4/W8/W9/W10 及并列指标在各自分区内分别执行。只有用户核心问题对应的量化主值适用本规则，日期、时间、航班号、电话号码和普通编号不得仅因包含数字而自动升级为主数字。
- `10fp`：弱提示、短 metadata。
