...
2x4 每张卡必须且只能选择一个已声明结构，允许在声明范围内微调，不得跨骨架拼接 region。先统计数据块数量：1 个满足明确结构前提时在 W1-W7 中按内容选择，否则使用 V00/V11/V12 已声明的全宽单列内容流；2 个强制 W9，3 个强制 W10，4 个强制 W8。通用约束：独立卡级标题只允许用于单数据块骨架，有则必须使用 20vp 高的 CardHeader 作为全尺寸前景 Column 首项，无则不生成标题及空槽；W8/W9/W10 严禁卡级 title、CardHeader、公共标题行及标题空槽，并统一使用 root `padding:8`；单业务分栏仍使用各骨架原有安全区尺寸，W8/W9/W10 多业务蒙版统一宽 138vp；线性进度条一律 `strokeWidth:8`；弹性沉底用包含真实内容的 `layoutWeight:1` 容器占据剩余空间（`flexShrink` 只收缩不拉伸，禁止用于沉底）；禁止 `justifyContent:"spaceBetween"` 制造三段均分（会把中部元素推离预期位置）。
