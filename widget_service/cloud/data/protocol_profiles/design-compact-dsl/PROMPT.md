...
**2x4 双业务生成前置约束**：必须先按业务对象实例合并字段，再统计数据块；同一耳机对象的名称、连接状态、耳机仓电量和充电状态固定算一个数据块，禁止拆成“耳机连接”和“耳机仓电量”两个数据块。最终展示恰好两个业务数据块时先判断主次：存在一个明确主焦点、其余必要内容能压入两个辅助槽时锁定 W1-focus-aux；只有两个业务等权且都需要完整内容区时才锁定 W9。W9 root 必须是 Row、`padding:8`，直接且只能包含左右两个 `138×134vp` 大内容背板，间距 `8vp`；禁止复用 2x2 S4，禁止 `root Stack -> content Column`，禁止两个全宽背板上下排列，禁止任何其它上下双业务布局。禁止卡级公共标题、跨业务公共内容行、卡级 `actions/action_area` 和背板外按钮。每个业务的标题、数据和至多一个按钮必须全部放在自己的背板内，action 数量不增加、拆分或重排业务数据块，也不得为了容纳 action 改走 W10。W9 大背板带动作时直接子节点固定为 `[content, action]`，`content` 必须 `layoutWeight:1`，action 必须是最后一项并固定 `114×36vp`。纯文字 action 使用 Button；图文 action 使用 `Row -> [Image, Text]`，Row 必须显式写 `itemMargin:8`、左右 `padding:8`、`justifyContent:"center"`、`alignItems:"center"`，Image 固定 `20×20vp`。`content` 通常最多三行视觉信息；数字与单位拆成两个 Text 时仍属于同一行，多日天气允许使用“标题 + 三条日期摘要”的四行紧凑内容。辅助字段只有在两个短事实能够完整显示时才用 ` | ` 合并；三个以上动态字段不得塞进同一窄行，优先拆行，仍超出再删除低优先级可选字段。
