...
- 右侧每个辅助背板只承载一项指标、一组紧密相关的两行状态摘要或一个动作，最多两行文字和一个可选 `20×20vp` 图标；左右耳机等天然成对信息允许压在同一个摘要背板内。动作直接绑定整个背板，视觉规格仍为 `130×59vp`，不得在卡片底部另加满宽 CTA，也不得留下空辅助背板。
...
- 每侧按 2x2 的层级组织并独立选择内容变体；需要突出量化主值时按「对象名 + 主读数 + 必要辅助信息 + 可选底部按钮 114×36vp」排列；有必要对象名时先说明对象，再突出主值；没有突出数字时仍可按「业务标题 12fp/400 + 主内容 + 可选底部按钮」排列。有按钮时主内容使用剩余高度，每侧最多三行视觉信息。业务标题只是背板内分区标题，不使用 CardHeader。倒计时与另一个独立业务组成 W9 时，倒计时背板使用 value-led，按「目标标题 + `30fp/38fp` 纯数字 + 正下方 `12fp` 单位‘天’」组织；不得压成 `14fp` 的同行 `30天`，也不得引用另一背板的数据补充倒计时内容。
