...
**2x4 双业务生成前置约束**：必须先按业务对象实例合并字段，再统计数据块；同一耳机对象的名称、连接状态、耳机仓电量和充电状态固定算一个数据块，禁止拆成“耳机连接”和“耳机仓电量”两个数据块。最终展示恰好两个业务数据块时立即锁定 W9，root 必须是 Row，直接且只能包含左右两个 `134×126vp` 大内容背板，间距 `8vp`；禁止复用 2x2 S4，禁止 `root Stack -> content Column`，禁止两个 `276×59vp` 背板上下排列，禁止任何其它上下双业务布局。禁止卡级公共标题、跨业务公共内容行、卡级 `actions/action_area` 和背板外按钮。每个业务的标题、数据和至多一个按钮必须全部放在自己的背板内，action 数量不增加、拆分或重排业务数据块，也不得为了容纳 action 改走 W10。W9 大背板带动作时直接子节点固定为 `[content, action]`，`content` 必须 `layoutWeight:1`，action 必须是最后一项并固定 `110×36vp`。纯文字 action 使用 Button；图文 action 使用 `Row -> [Image, Text]`，Row 必须显式写 `itemMargin:8`、左右 `padding:8`、`justifyContent:"center"`、`alignItems:"center"`，Image 固定 `20×20vp`。`content` 最多三行视觉信息；数字与单位拆成两个 Text 时仍属于同一行。辅助字段过多时先用 ` | ` 合并同级短字段，仍超出则删除低优先级可选字段。
...
2x4 每张卡必须且只能选择一个骨架，允许在声明范围内微调，不得跨骨架拼接 region。先统计数据块数量：1 个在 W1-W7 中按内容选择，2 个强制 W9，3 个强制 W10，4 个强制 W8。通用约束：独立卡级标题只允许用于单数据块骨架，有则必须使用 20vp 高的 CardHeader 作为全尺寸前景 Column 首项，无则不生成标题及空槽；W8/W9/W10 严禁卡级 title、CardHeader、公共标题行及标题空槽；分栏一律使用骨架规定的新安全区尺寸（136/134/130），侧栏固定宽统一 130vp；线性进度条一律 `strokeWidth:8`；弹性沉底用包含真实内容的 `layoutWeight:1` 容器占据剩余空间（`flexShrink` 只收缩不拉伸，禁止用于沉底）；禁止 `justifyContent:"spaceBetween"` 制造三段均分（会把中部元素推离预期位置）。
...
- region：无卡级标题；使用 `276×126vp` 的 `2×2` 网格，两行两列的横纵 itemMargin 均为 8，每个小内容蒙版固定 `134×59vp`、padding 12、borderRadius 16、白色 80% 背景，并完整遵守 8.3 节小内容蒙版的两行数据、右侧图标和数据删减规则。仅在蒙版内有 1-2 项信息且需要比例视觉时允许使用右侧 `40×40vp、strokeWidth:4` 的环形 Progress 与环心 20×20vp 图标；3 项信息时不得保留环图，否则不得为填充版面强加环图。
...
- region：无卡级总标题；`root Row` 以 `width/height:matchParent` 填满 `300×150vp` 画布，padding 12、itemMargin 8，直接包含左右两个 `134×126vp` 同级背板。两背板均为 Column、padding 12、borderRadius 16、白色 80% 背板，内部可用空间均为 `110×102vp`。这是 2x4 双业务唯一合法的一级布局，不得套用 2x2 S4 的上下两个 `134×63vp` 结构，也不得放大为上下两个 `276×59vp` 背板。
- 每侧按 2x2 的层级组织；需要突出量化主值时按「对象名 + 主读数 + 必要辅助信息 + 可选底部按钮 110×36vp」排列；有必要对象名时先说明对象，再突出主值；没有突出数字时仍可按「业务标题 12fp/400 + 主内容 + 可选底部按钮」排列。有按钮时主内容使用剩余高度，每侧最多三行视觉信息。业务标题只是背板内分区标题，不使用 CardHeader。W9 中的倒计时必须写成同一行普通主数据（如 `30天`），固定 `14fp/700`，禁止使用单业务倒计时的 `30fp/38fp` hero，也禁止把单位“天”拆成独立一行。
...
- `20fp`、`24fp`：紧凑纯数字；`30fp`：主焦点纯数字默认档；`32fp`、`38fp`：内容少且空间充足时的数字放大档，数字最大 38fp。S1 `centered-focus` 是受控例外：短文字主信息固定 20fp，纯数字主信息固定 38fp。
...
**格式化主读数例外（与转换前校验一致）**：适用于明确描述为温度、时长或百分比的主读数，不限于单一 data 根；名称、日期、时间、状态不适用。内容可以是直接 PathBinding，也可以是只引用一个字段并追加真实单位的简单 Expression，不得拼接名称、状态或说明。Text 必须单行、无 padding/margin、显式高度至少为字号的 1.4 倍，并与实际主内容槽同宽：2x2 使用 126vp，2x4 全宽使用 276vp，W9/W10 左侧大背板等 134×126vp 区域使用内部 110vp；父级 Column/Row 必须同宽且无 padding。只允许 20fp 或 24fp；2x2 的 126vp 内容槽和 W9/W10 的 110vp 内容槽默认使用 20fp，只有更宽的内容槽通过压力预算后才可使用 24fp。须以较长合法值（负温、100%、两位数小时和分钟）加 20% 余量检查宽度。W8、小内容蒙版与环中心不使用该例外；不能通过增大 Text 声明宽度突破父级预算。
...
- 动态长名称、会议名、设备名和“可点击入口/对象名称”类内容（音乐入口、设置项、蓝牙设备名、联系人）不用 30fp 大字；改短静态主文案、放小字，默认 `fontSize:16`，内容少且空间充足时最大 `18fp` 并让 Text 占整行。只有 S1 `centered-focus` 的唯一短文字主信息允许使用 `20fp/700`，标题和辅助文字仍为 12fp。
...
- `2x4` 底部动作区只有一个卡级 Button 或图文按钮 Row 时，该按钮固定为 `x:12vp、y:102vp、width:276vp、height:36vp`，距卡片底部必须正好 `12vp`；按钮必须是满高前景 Column 的最后一项，上方真实内容区使用 `layoutWeight:1` 占据剩余空间。禁止生成 `126/130/134vp` 的半宽单按钮或浮动底边距。W9 和 W10 左侧大背板内按钮是例外，固定宽 `110vp` 并留在所属业务背板内。
...
4. **骨架与预算**：是否保留每个并列对象的名称和有歧义指标的标签；必要文字是否使用主文字色而非 60%；普通 S2 是否主信息顶对齐、描述紧跟、动作沉底；是否只使用一个固定骨架；单业务双按钮是否为 S3 的 40vp 信息组加 78vp 动作组，主信息是否合并为 14fp 且没有独立标题和大数字内容区；是否按最终展示对象正确统计数据块，2x4 的 1/2/3/4 个数据块是否分别路由到 W1-W7/W9/W10/W8，2x2 恰好两个数据块是否强制使用完整双分区 S4，单数据块是否未生成单个 `134×63vp` 内容背板或残缺 S4，且没有把任一数据块降级为另一数据块的辅助信息；2x2 S4 两个背板内的文字、可染色图标、Progress 和 Divider 是否统一使用同一主内容色 RGB；所有小内容蒙版是否只有两行、左文字右 visual，1-2 项如有右侧图标，是否语义准确且状态安全，3 项时是否去掉 visual 并以 ` | ` 合并辅助数据，超过 3 项是否删除低优先级项，图标或环外框是否距蒙版右边 12vp；CardHeader 是否只用于 2x2 S2 或 2x4 单数据块的独立卡级标题、每卡最多一个且位于规定前景 Column 首项，W8/W9/W10 是否没有卡级标题、CardHeader、公共标题行或标题空槽，无标题布局是否未留空槽，S1/S3/S4 是否未加 CardHeader 或右上角辅助图标；root 宽高是否为 `"matchParent"`、普通布局 padding 12、S4 padding 8、圆角 20、clip true；root Column 是否使用 `alignItems:"center"`，root Row 的固定参考宽度直接内容是否使用 `justifyContent:"center"`，且没有把 root 的整组居中误写成内部文字全部居中；是否已按 3.1 节逐项写出 root 直接子节点的最小高度、margin 和 `itemMargin` 并确认普通布局 `H_required <= 126vp`、S4 `H_required <= 134vp`；分布式对齐是否只分配扣除最小间距后的非负剩余空间；所有 Row/Column 两轴预算是否非负，动态文字 Row 是否保留余量，点击热区是否至少 24vp。
