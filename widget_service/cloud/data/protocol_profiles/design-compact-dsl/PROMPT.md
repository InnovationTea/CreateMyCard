...
- 候选存在只表示“允许使用”，不表示用户已经要求交互。必须先把语义匹配的事件分为以下三级，再决定是否保留：
  ...
  1. **显式动作 `explicit`**：用户明确使用“打开、进入、查看、导航、拨打、开启、关闭、设置、清理、播放、暂停、执行”等动作表达，并且候选目标与动作对象一致。该事件属于 `mustKeep`，必须绑定到可见且语义合适的组件；用户明确要求按钮时必须提供可见 CTA，S4 双业务按第 9.1 节直接绑定对应蒙版分区，不单独呈现 CTA。
  2. **隐式入口 `implicit`**：用户只要求信息展示，但存在与同一服务对象严格一致、无副作用的“打开 App、进入同主题详情、查看完整信息”入口。该事件属于 `shouldKeep`，可以作为整卡唯一入口绑定 root；S4 双业务改为绑定所属蒙版 zone。它不得挤占核心内容，也不得自动生成额外按钮。用户明确要求纯展示、不可点击时必须舍弃。
  3. **副作用动作 `sideEffect`**：会改变系统或应用状态、发起通信、开始导航、删除或清理数据、购买或提交、控制设备、播放或暂停等行为。只有用户明确要求该动作时才能使用；仅因候选存在、主题相近或“可能有用”不得推断执行意图。

- 事件入口选择遵循动作性质：无副作用的单一隐式入口优先绑定 root，S4 双业务绑定所属蒙版 zone；显式且需要确认操作目标的动作使用可见 Button 或 clickable Row，S4 同样直接绑定所属 zone；图标独立动作必须具有清晰 accessibility.label。
...
7. **动作闭环**：Button 或专用 clickable Row 的可见文案只表达动作本身，默认压缩为简短的“动词 + 对象”；S4 可点击蒙版仍展示所属业务内容，不额外生成动作文案。任何含“导航、打开、查看、清理、开启、关闭、拨打”等动作语义的按钮外观都必须具有合法 `onClick`；否则删除动作措辞和按钮外观。
...
- 纯文字按钮使用 Button；图文按钮使用一个带 `onClick` 的 Row，内部组合 Image 和 Text；被选中的无副作用单一隐式入口优先放在 root，不额外占用版面，S4 双业务则绑定所属蒙版 zone。同一动作只选择一个点击容器，不重复绑定。
...
- `2x2` 中带文字的主动作优先使用底部全宽 Button 或全宽图文 Row；S4 双业务不生成独立动作区，事件直接绑定所属蒙版分区。不得把 Image + Text 横向塞进窄于 `56vp` 的侧边动作栏；空间不足时改成全宽动作、纯文字 Button，或仅保留带 accessibility.label 的独立图标动作。
...
- `2x2` 最多 3 个主区域，默认最多 1 个显式动作；S4 双业务允许每个蒙版分区各绑定 1 个属于自身业务的动作。
...
- region：只允许上下纵堆，固定为 `root -> [zone_top, zone_bottom]`，root `itemMargin:8`；两 zone 均为 `136×64vp`、borderRadius 16、内容色 10% 蒙版，且 `64 + 8 + 64 = 136`，不得增加 title/header/footer/action_area，也禁止左右双业务布局。zone 内以文字组为主，只有需要视觉主体时才使用 `Row -> [文字组, 视觉主体]`。
- 两块蒙版 Row/Column 必须显式写 `padding:{left:12,right:12,top:0,bottom:0}`。内部可用宽度为 `136 - 12 - 12 = 112vp`；独立 20vp 图标与文字间距 8vp 时，文字列及其 Text 最多宽 `112 - 20 - 8 = 84vp`，不能沿用无蒙版布局的 88/136vp。左侧图标距 root 左边 24vp；右侧图标贴内部右边缘，距 root 右边 24vp（左坐标 116vp）。内层 Row 最多宽 112vp；环图按环外框计算预算，图标仍在环中心，不单独移位。不能漏掉蒙版、内边距或用 root 的 padding 代替。
...
- 浅色纯色背景：2x2/2x4 的 Button、ActionUnit 和专用图文按钮 Row，按钮背板都固定为本卡内容色 20%，按钮文字及单色图标固定为内容色 100%；S4 即使带 `onClick` 仍是内容蒙版，保持内容色 10%，不套按钮配色。禁止内容色 100% 实心背板配白色文字或白色图标，也不能换成不透明浅面、白底或另取更深的按钮文字色。
...
3. 为骨架填入角色槽位，确定共同对齐线、主辅面积和留白；只保留最小充分数据、事件和素材。显式动作必须落到合法 CTA，S4 除外并直接绑定所属蒙版 zone；隐式入口只能作为不抢占空间的 root 入口，未被显式要求的副作用动作必须删除。
...
3. **候选与事件**：是否只保留最小充分候选；显式动作是否绑定，隐式入口是否不抢占空间，未被明确要求的副作用动作是否已删除；同一动作是否只有一个点击容器；S4 的 root 是否没有 `onClick`，每个事件是否只绑定所属蒙版 zone。
