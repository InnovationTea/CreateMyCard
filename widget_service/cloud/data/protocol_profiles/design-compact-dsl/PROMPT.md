...
**2x2 单业务多字段生成前置约束**：`/data` 下只有一个一级业务根时始终是单业务；同一根内的名称、状态及多个指标不得按字段数拆成 S4。禁止生成单个或残缺的 `134×63vp` S4 内容背板，使用全宽单业务信息流。用户明确要求展示且不用于动作参数的 1-3 个不同字段必须全部保留并各显示一次；“重点、优先、主要”只调整字段顺序和视觉主次，不得删除其余明确字段。重点字段放在前面。为适配 `150×150vp` 画布，普通 S1/S2 的全部信息区统一使用以下三类密度：①单一数字焦点含 `30fp/38fp` 大数字时，数字与真实单位合计为一行，其余字段最多组成一条 `12fp/400` 辅助信息行；辅助行可以是一个完整 Text，也可以是一个横向 Row 内的多个 `12fp/400` Text，使用 ` | ` 或等价的明确间隔分隔，各指标必须保留必要标签或自解释单位并共同通过单行宽度压力检查；②单一文字焦点最多一行突出文字和两行 `12fp/400` 辅助信息；③两个及以上同级并行指标不得制造大数字焦点，每项压成一条普通字号完整信息，最多三行。视觉行按实际 Row/Column 结构计算，不按 Text 节点数量累加；Button、ActionUnit 和带 `onClick` 的图文动作 Row 都属于 action，不计入信息行。V01、V06、S3、S4、含 Progress 的视觉亚型及 2x4 W8/W10 小背板不适用本规则。2x4 W9 的左右两个 `138×134vp` 大背板必须分别把内部业务标题和 action 排除后独立执行同一密度规则，不能因位于 2x4 内而恢复额外信息行或多个突出焦点。普通单业务内容区不生成 Image，不得为使用候选素材套用 S4 小背板。

**单业务并行指标生成前置约束**：大数字 `value_row` 只能包含纯数字 Text 和紧邻的真实单位 Text，禁止在该 Row 内放入标签、方向、状态、名称、说明或其它字段；这些信息必须放在主值下一行。`value_row` 固定使用 `alignItems:"bottom"`，单位 Text 不设置与数字相同的固定高度，并使用 `padding:{"bottom":4}` 校正不同字号的可见基线。主值绑定 string、名称或状态时不得生成相邻单位 Text，例如 `exerciseTypeName="户外跑步"` 后不能再追加“跑步”。同一业务对象内存在最高/最低、当前/目标、已用/剩余、左/右等两个及以上同级量化指标时，所有指标必须上下排列，禁止用 Row 拆成左右两列或左右两个指标组；同时禁止选中其中一项使用 `30fp/38fp` hero、数值与标签混合字号 Row 以及多个大数字 `value_row`，每个指标必须压成一个完整的 `12fp/400` 单行 Text，按“短标签 + 数值 + 单位”展示，并保持相同字号与字重。此规则适用于 2x2 普通 S1/S2，也适用于 2x4 W9 的每个独立大背板；不适用于 V01、V06、S3、S4 和 2x4 W8/W10 小背板。
