...
- `2x2` 默认最多展示 2 个图标，不必凑满，所有位置的图标统一 `20×20vp`。按最终实际展示实例计数：Image 图标、ActionUnit.icon、CardHeader.icon、环内及重复图标都算；进度环本身、融球背景和 L/R/盒等 Text 文字标识不算图标，文字标识保持原尺寸。
- `2x2` 辅助小字区禁止使用图标，辅助指标、状态说明、时间、地点等只用文字，不逐行套用「Image + Text」；按信息职责判断，不得通过增大字号或改名为主内容绕过。用户明确指定优先，仅对指定部分放宽图标区域、数量或尺寸，其余仍严格执行本节。
...
- `2x2` 图标区域强互斥，布局前按「内容区 > 按钮区 > 标题区」只选一个区域使用图标：内容区有图标（含环内及 bottom_area 的内容图标），按钮区和标题区都不配；内容区没有且按钮区有图标，标题区不配；只有前两区都没有图标时，标准 CardHeader 才可配右上角图标。不向低优先级区域补齐剩余名额，不为凑数添加内容图标；用户明确指定部分除外。无图标标题不写 CardHeader.icon，无图标动作从布局前采用纯文字并保留动作，不生成 icon-round、图文按钮或空图标槽。
...
- 图文按钮是正式支持的交互形态。当用户明确需要图文按钮，或已满足 2.5 节区域互斥且匹配图标能明显提升动作识别时，使用一个带 `onClick` 的 Row 作为完整按钮容器，Row 内放 Image 和 Text；不得因 Button 不支持图标而删除用户指定图标，也不得给 Button 增加协议外图标字段。
...
- 亚型：数值亚型（`value_row` 数字+单位 + 进度条/辅助行）；状态亚型（状态文字列 + 辅助行）；视觉亚型（`root -> [title_area, content_area, bottom_area]`，`bottom_area Row -> [ring_icon_stack, action_area]`，其中 `ring_icon_stack` 为环形 Progress 与中心图标或读数的叠放组合，左下展示状态视觉；使用环内或其他内容图标时，右下动作预先采用纯文字入口并预算文字宽度，标题不配图标；icon-round 仅在满足 2.5 节区域互斥及用户指定例外时选用）。
- 槽位：标题、两行信息、至多一个显式动作。
...
- 亚型：纵堆（`root -> [zone_top, zone_bottom]`，root `itemMargin:8`，两 zone 各高 64、borderRadius 16 的同族低对比背板，`64 + 8 + 64 = 136`，不得再增加 title/header/footer；zone 内 `Row -> [文字组, 视觉主体]`，其中一方可承载动作入口）；横行（`root -> [title_area?, content_area]`，`content_area Row -> [visual_group, text_group]`，visual_group 的 ring/Stack 为 48-52vp，独立或环内图标统一 20×20vp，text_group 为文字列）。
...
- `2x2` 顶部第一行文字信息若不是 CardHeader，必须为纯文字，不在文字前后或该行右侧配图标；无论它被命名为标题、倒计时、状态或业务说明，也无论左对齐、居中、单业务或多业务，都执行本条。例如「北京出差还有 30 天出发」不生成飞机 Image 或图标槽。其他居中标题或说明行同样只用文字；用户明确要求图标时除外。不得将首行前缀图标解释为内容区主视觉来绕过本条；独立主视觉、进度环内图标及图文按钮仍按原规则和 2.5 节总名额执行。
- 标题图标仅在已分配图标名额时显示，固定 `20×20vp`，通常位于标题行右侧；在 `2x2` 标题行中必须贴安全区右上角，右边缘距 root 右边 `12vp`。
- `2x2` 的标题、按钮、主内容和进度环内图标全部固定 `20×20vp`，不再使用其他尺寸档位；用户明确指定尺寸时才覆盖，不改变按钮点击外框或进度环尺寸。高阶组件不能表达用户指定尺寸时，用对应基础组件布局，不给宏组件添加未定义字段。
