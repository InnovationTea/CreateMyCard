...
- 字号：主信息是纯 number/integer 时，数字固定 `38fp/700`，真实单位固定 `12fp/400`；主信息是字符串或带单位格式化值时固定 `20fp/700`。唯一辅助信息固定 `12fp/400`。标题、主信息和辅助信息均为单行居中。
