...
- 原始数值与单位使用完整 Expression 时，单位必须作为独立字符串项参与拼接，例如 `"{{ ${/data/countdown/countdownDays} + '天' + '后开始' }}"`。不得省略单位，也不得用近似字符、同义词或说明文案代替声明单位。
- 原始数值与单位使用多个 Text 做视觉分层时，数值 Text、单位 Text、补充说明 Text 必须是同一 Row 中按顺序相邻的独立节点；单位 Text 的静态 `content` 必须精确等于声明单位。需要表达“30天后开始”时，结构必须是“动态数值” + `"天"` + `"后开始"`，禁止把后两项合并成相邻 Text `"天后开始"`。
- 主数值需要大字号、单位和说明需要小字号时优先采用拆分 Text，保持同一 Row 底对齐并分别完成宽度预算；不需要视觉分层时才使用单个完整 Expression。不得为了通过单位规则增加重复单位、空 Text 或不自然的说明文案。
...
- 温度单位优先统一写作 `°C`。数值型摄氏温度按 4.2.1 由精确单位 Text 或完整 Expression 补充 `°C`，不得使用单独的 `°`、圆点、实心圆或其它近似符号冒充温度单位；若 TaskSpec 提供的格式化字符串已经包含单位，则完整绑定原值，不重复追加或擅自改写。
...
5. 空间不足时优先把次要状态移到主值下方、扩大主值槽位或降低到批准字号。不要从已经格式化且自带单位的动态字符串中剥离单位另造静态 Text；原始 number/integer 字段需要按描述补充静态单位时，允许并优先按 4.2.1 拆分数值、精确单位和补充说明。
6. 不给动态字段追加重复或可由其自身表达的同义后缀。例如天气现象已显示“小雨”时，不再拼接“· 降雨”；按钮已写“导航回家”时，不再增加“点击”或“立即”。静态拼接只有在增加独立信息维度时才保留。

数字与单位拆分：

- 原始数值需要补充静态单位且存在字号层级时必须拆开，同一 Row 底对齐：无补充说明时使用 `value_row -> [value_num, value_unit]`；有补充说明时使用 `value_row -> [value_num, value_unit, value_hint]`。`value_unit.content` 必须精确等于声明单位，禁止写成“单位 + 说明”的合并文案。
- `value_row` 只在 Row 上写总宽度；`value_num`、`value_unit` 不写 `width:"matchParent"`，两者按内容自然宽度并写 `flexShrink:0`，避免右侧单位被挤出卡面。
- 内容区同一 Row 内两个及以上 Text 统一 `alignItems:"bottom"`；`value_unit` 字号 12-16，单位不能用 30 号字；`value_hint` 与 `value_unit` 使用相同或更弱的字号、字重和颜色，不得争夺主数值焦点。
