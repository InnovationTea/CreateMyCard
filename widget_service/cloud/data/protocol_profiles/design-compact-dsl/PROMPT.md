...
**2x2 单业务双动作生成前置约束**：`/data` 下只有一个一级业务根且恰好有两个动作时立即锁定 S3。root 只能包含 `136×48vp` 的 `header_area` 和纵排两个 `136×36vp` ActionUnit 的 `action_area`，两区间距 `8vp`；禁止 title_area、content_area、value_group 和 value_row。信息区固定两行：第一行是业务或对象名称 `14fp/700`，第二行将全部保留数据合并为一个 `12fp/400` 单行 Text，多个数据用 ASCII ` | ` 分隔。数字也按普通文字展示，禁止任何 18fp 及以上字号、hero 数字、第三行信息或数值与单位的大小字号组合。两个动作不增加业务对象数量；存在两个业务对象时仍优先执行 S4。
...
- S4 双业务按两个独立展示对象划分，不要求来自不同业务类型或数据能力；两座城市的天气也算双业务。同一天气对象的温度、湿度、空气质量等多个字段不算双业务。每个对象区最多 1 个主视觉图标（环中心也计入），辅助文字前后仍不配图。两个天气对象各有合法素材时，每区各放 1 个天气图标；允许复用同一候选 src，但分别计数。不得为放图标新增进度环或把单对象字段拆成双业务。不得按文字长度、字符数、是否超过 6 个字或是否占用一行/两行来删除、移动或缩放 S4 主视觉图标。
...
- 小内容蒙版统一指 2x2 S4 的 `136×64vp` 分区和 2x4 W8/W10 的 `144×64vp` 分区，只允许两行内容：第一行主数据固定 `14fp/700`、内容色 100%，第二行辅助数据固定 `12fp/400`、内容色 60%，不再增加独立业务标题行；需要突出量化主值时，第一行必须以该数字及其单位为主，不在数字前添加业务标签。蒙版内 1 项数据时只输出第一行，2 项时分别放第一、二行；有合法候选素材时，1-2 项必须使用一个右侧图标，无合法素材时不得伪造或留空槽。文字字符数和行数只用于文字自身的删减与压力检查，不参与图标取舍或定位；禁止使用“超过 6 个字”等长度阈值决定图标。3 项时不使用任何右侧视觉，第一行放主数据，第二行将两个辅助数据写成单个 Text 并以 ` | ` 分隔；超过 3 项时按“用户明确要求 > 主数据 > 状态 > 其他辅助”只保留前三项，其余删除。
- 小内容蒙版使用图标时固定为 `Row -> [text_column, visual]`，即使只有一行文字也必须用定宽 `text_column` 包装；父 Row 的 `itemMargin` 只能是 `8vp`，不得按通用组间距改成 10/12/16vp，也不得省略或写 0。visual 固定在右侧并垂直居中，右边缘距蒙版右边固定 12vp，不得因文字长短改变 children 顺序、文字列宽、间距或 visual 位置。独立 Image 固定 `20×20vp`：2x2 文字列宽 84vp，并满足 `12 + 84 + 8 + 20 + 12 = 136`；2x4 文字列宽 92vp，并满足 `12 + 92 + 8 + 20 + 12 = 144`。仅有 1-2 项信息，且目标字段通过 11.3 节 Progress 比例语义与可靠 `total` 检查时，visual 才可以是右侧环图；否则只能使用合法 Image 或删除 visual，严禁把任意数值包装成环。合法环按环外框右边缘距蒙版右边 12vp：2x2 S4 使用 `44×44vp` 环和 60vp 文字列，2x4 W8 使用 `40×40vp` 环和 72vp 文字列；中心图标保持 20×20vp。3 项时必须删除包括 Image、Progress 和环中心内容在内的整个 visual；无合法图标且无合法环图时同样删除 visual，文字列占满蒙版内部宽度。
...
提示词末尾若存在“本次请求固定场景路由（最高优先级）”，必须先执行该路由锁并跳过本节通用对象计数与骨架选择；不得再根据字段数量、`/data` 一级节点或普通数字规则改选其它骨架。V01 倒计时锁只允许用于 `/data` 下恰好一个业务根的单业务请求；只要存在第二个一级业务根，就不得追加或执行 V01 锁，必须按多业务骨架路由。没有路由锁时才执行以下通用路由。
...
- region：`header_area` 固定使用剩余的 `48vp` 信息区并承担标题职责；其后间隔 `8vp`，`action_area Column -> [cta_1, cta_2]` 使用两张 `36vp` ActionUnit 纵排、`itemMargin:8`、沉底；无独立 title_area、content_area、value_group 或 value_row。
- 槽位：第一行固定为业务或对象名称 `14fp/700`；第二行固定为业务数据摘要 `12fp/400`，所有保留数据必须合并在这一行，多个数据用 ASCII ` | ` 分隔。两个动作必须都有精确候选且由用户明确要求。S3 的设备名属于主内容，不使用 CardHeader，也不放名称旁的右上角辅助图标。
- 禁止：三按钮、双信息域、第三行信息、任何 18fp 及以上字号、hero 数字、数值与单位的大小字号组合。

### `S4-stacked-zones`（上下双业务）

- 用于：两个独立展示对象的并列分区，可为不同业务（天气+打车、内存+耳机），也可为同类业务（两座城市的天气）；不能将同一对象的多个字段当成双业务。
- 倒计时与 calendar、天气或任一其他一级业务节点共同展示时仍是两个业务对象，必须使用 S4。选定 S4 后禁止再提取任何单业务倒计时排版；倒计时只作为所属 zone 第一行 `14fp/700` 普通加粗主数据，例如 `4天`，不使用 30fp/38fp 大数字、800 字重、V01 居中 hero、独立 `value_group`、公共标题或底部按钮。
...
- region：可选 `CardHeader 20vp` + `body 108`：`progressSlot 50`（主数值行 20fp/700 + label 12fp/400 + `Progress linear 296×8 strokeWidth 8`）+ `details Row 50`（两背板 144×2 gap 8、圆角 10、内容色 10% 底，各「标题 12fp/400 + 值 12fp/400」）。
- 槽位：一个进度主指标 + 两个详情项。
...
- region：无卡级标题；使用 `296×136vp` 的 `2×2` 网格，两行两列的横纵 itemMargin 均为 8，每个小内容蒙版固定 `144×64vp`、padding 12、borderRadius 16、内容色 10% 背景，并完整遵守 8.3 节小内容蒙版的两行数据、右侧图标和数据删减规则。仅在蒙版内有 1-2 项信息且需要比例视觉时允许使用右侧 `40×40vp、strokeWidth:4` 的环形 Progress 与环心 20×20vp 图标；3 项信息时不得保留环图，否则不得为填充版面强加环图。
...
- region：无卡级总标题；`root Row` 以 `width/height:matchParent` 填满 `320×160vp` 画布，padding 12、itemMargin 8，直接包含左右两个 `144×136vp` 同级背板。两背板均为 Column、padding 12、borderRadius 16、内容色 10% 背板，内部可用空间均为 `120×112vp`。这是 2x4 双业务唯一合法的一级布局，不得套用 2x2 S4 的上下两个 `136×64vp` 结构，也不得放大为上下两个 `296×64vp` 背板。
- 每侧按 2x2 的层级组织；需要突出量化主值时按「数字主值第一行 + 业务文字/辅助信息 + 可选底部按钮 120×36vp」排列，业务文字不得放在数字前；没有突出数字时仍可按「业务标题 12fp/400 + 主内容 + 可选底部按钮」排列。有按钮时主内容使用剩余高度，每侧至多保留两行最小充分信息。业务标题只是背板内分区标题，不使用 CardHeader。
...
- region：无卡级标题；`root Row` 以 `width/height:"matchParent"` 填满 `320×160vp` 画布，padding 12、itemMargin 8，直接包含左侧 `144×136vp` 大内容蒙版和右侧 `144×136vp` Column。右侧 Column 使用 itemMargin 8，上下各放一个 `144×64vp` 小内容蒙版。三个内容蒙版均使用 padding 12、borderRadius 16、内容色 10% 背景。
- 左侧大内容蒙版需要突出量化主值时按「数字主值第一行 + 业务文字/辅助信息 + 可选底部 Button 120×36vp」组织，业务文字不得放在数字前；没有突出数字时仍按「业务标签 12fp/400 + 主内容 + 可选底部 Button」组织。有动作时只绑定内部按钮，大内容蒙版本身不绑定，主内容最多两行最小充分信息。右侧两个分区完整遵守 8.3 节小内容蒙版的两行数据、右侧图标和数据删减规则。
...
- 需要突出量化主值时，数字及其紧邻单位必须位于所属业务主内容区第一行，其他需要突出的文字放在数字下方。CardHeader 和会议日期上下文不计入主内容行；2x2 倒计时是位置例外，目标名称/标题在顶部居中，数字主值组在其下方的卡片中部，有动作时动作区仍沉底，禁止把数字移到标题上方；环形 Progress 视为主内容第一项；S4/W8/W9/W10 及 2x4 明确允许的并列指标骨架在各自分区内分别执行。2x2 单业务的多个同级指标不得并列，必须按优先级上下排列。只有用户核心问题对应的量化主值适用本规则，日期、时间、航班号、电话号码和普通编号不得仅因包含数字而自动升级为主数字。
...
- 未拆分的数字与单位字符串（如 `7小时30分`、`29°C`、`82%`）按文字处理，默认 16fp、最大 18fp，但作为量化主值时仍必须放在所属业务主内容区第一行；拆分后纯数字最大 38fp，单位使用 12-16fp。S3 双按钮骨架空间有限，所有数据固定合并在第二行并使用 `12fp/400`，不使用 hero 数字或拆分单位。
...
同级内容必须使用同一字号、字重、尺寸和对齐方式；仅在 2x4 明确允许并列指标的骨架中，同一组并列指标不能因为某项文本较长就单独降字号或改变垂直位置。某一项放不下时，应统一降低该组字号、统一调整槽位、缩短弱标签或改用更合适的骨架。2x2 单业务不得引用本条生成左右并列指标。
...
- 原始数值需要补充静态单位且存在字号层级时，优先在同一 Row 中拆成 `value_row -> [value_num, value_unit]` 并底对齐；`value_unit.content` 只能写声明单位本身。“后开始”“已使用”等说明文字必须另起一行。`value_row` 只服务于一个指标，不得包含第二个字段、第二个 `value_row` 或第二个指标 Column；2x2 单业务不得把多个 `value_row` 放进同一个横向 Row。
...
- `2x4` 底部动作区只有一个卡级 Button 或图文按钮 Row 时，该按钮固定为 `x:12vp、y:112vp、width:296vp、height:36vp`，距卡片底部必须正好 `12vp`；按钮必须是满高前景 Column 的最后一项，上方真实内容区使用 `layoutWeight:1` 占据剩余空间。禁止生成 `136/140/144vp` 的半宽单按钮或浮动底边距。W9 背板内按钮是唯一例外，固定宽 `120vp` 并留在所属业务背板内。
