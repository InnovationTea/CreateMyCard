...
**2x2 双业务生成前置约束**：最终展示恰好两个业务对象时立即锁定 S4，root 固定 `padding:8`，直接子组件只能是上下两个 `134×63vp` 内容背板且间距 `8vp`，禁止标题区、底部 `action_area`、Button、ActionUnit 和 TimelineUnit；每个保留动作只能绑定所属内容背板，即使其中一个业务是带 `EnterMeeting` 的会议也不得生成独立“加入会议”按钮，也不得在背板内生成“点击卡片”、“一键入会”等动作提示 Text。S4 会议背板固定第一行仅放会议标题 `14fp/700`，第二行仅放时间 `12fp/400`；无标题字段且用户未提供名称时使用“会议”，禁止将时间与标题拼成同一行。进入 S4 后全部文字只执行小内容背板规则，禁止继续提取 S1/S2/S3、V01 或 hero 规则。倒计时只作为所属背板第一行 `14fp/700` 的普通主数据。整张 S4 只能选择一套卡片色板：两个背板内的 Text、可染色 Image、Progress 和 Divider 必须使用相同的主内容色 RGB，只允许按角色改变 alpha，禁止按业务分别使用会议橙、设备青等不同主题色。每个背板最多两个 Text，且每个 Text 必须 `maxLines:1`，实际只能形成两行文字；1-2 行文字必须作为一个连续文字组在 `63vp` 背板内垂直居中，不得使用 `justifyContent:"start"` 顶对齐或为不存在的第三行预留高度。静态主标题若无法装入固定文字列，必须保留语义后缩短，例如“合作方评审会”缩为“评审会”，不得依赖换行、缩小字号或增加第三行。文字字符数、是否超过 6 个字不得决定图标是否存在或位置；1-2 项数据且有语义准确、状态安全的候选素材时保留一个右侧图标，固定 `20×20vp` 且距背板右边 `12vp`。只要背板包含 visual，背板就必须是 `Row -> [82vp 文字组, 20vp visual]`，固定 `itemMargin:8`、`justifyContent:start`、`alignItems:center`，左侧文字 Column 固定 `justifyContent:"center"`；严禁使用 Column 把 visual 排到文字下方。无 visual 时才允许背板使用 Column，文字宽度为 `110vp`，背板固定 `justifyContent:"center"`。

**2x2 单业务多字段生成前置约束**：`/data` 下只有一个一级业务根时始终是单业务；同一根内的名称、状态及多个指标不得按字段数拆成 S4。禁止生成单个或残缺的 `134×63vp` S4 内容背板，使用全宽单业务信息流。用户明确要求展示且不用于动作参数的 1-3 个不同字段必须全部保留并各显示一次；“重点、优先、主要”只调整字段顺序和视觉主次，不得删除其余明确字段。重点字段放在前面。为适配 `150×150vp` 画布，普通 S1/S2 的全部信息区统一使用以下三类密度：①单一数字焦点含 `30fp/38fp` 大数字时，数字与真实单位合计为一行，其余字段最多合并成一条 `12fp/400` 辅助信息行并以 ` | ` 分隔；②单一文字焦点最多一行突出文字和两行 `12fp/400` 辅助信息；③两个及以上同级并行指标不得制造大数字焦点，每项压成一条普通字号完整信息，最多三行。标题和 action 不计入信息行；V01、V06、V11、S3、S4、含 Progress 的视觉亚型及 2x4 W8/W10 小背板不适用本规则。2x4 W9 的左右两个 `138×134vp` 大背板必须分别把内部业务标题和 action 排除后独立执行同一密度规则，不能因位于 2x4 内而恢复额外信息行或多个突出焦点。普通单业务内容区不生成 Image，不得为使用候选素材套用 S4 小背板。

**2x2 单业务稀疏居中生成前置约束**：整卡只有一个业务对象、只有一个明确主信息和恰好一个辅助字段，并且没有任何保留事件或 action 时，使用 S1 `centered-focus` 亚型并参考 V11。辅助字段为零个或两个及以上时都不得使用本亚型，必须回到原有单业务骨架。root 固定 `padding:12`，一级顺序固定为顶部居中的 `title_area 28vp` 与占据其余空间的 `focus_group`；标题固定 `20fp/400`。`focus_group` 直接包含 `primary_area` 和底部 `supporting_text`。`primary_area` 使用 `layoutWeight:1` 将唯一主信息固定在中间，`supporting_text` 固定 `height:20`、`flexShrink:0` 沉底并只绑定该唯一辅助字段。纯 number/integer 主信息固定 `38fp/700`，真实单位另用 `12fp/400`；字符串或带单位格式化主信息固定 `24fp/700`；辅助信息固定 `12fp/400`。所有文字居中，不生成 Button、ActionUnit、Progress、Image、CardHeader、bottom_area、空单位 Text 或第三行信息。倒计时 V01、会议时间线 V06、S3 和 S4 不适用本亚型；一旦存在需要保留的 action，立即退出本亚型并使用对应的原有骨架。

**2x2 V11 排他约束**：只有 few-shot 路由明确选择 `centered-focus / V11` 时，才允许使用“居中标题 + 居中主信息 + 底部居中辅助文字”的整卡纯文字布局。未选择 V11 时必须完全忽略 V11 的结构和对齐方式，继续使用当前路由原有 few-shot；除 V01 倒计时外，普通单业务标题和纯文字内容组默认水平左对齐，使用 `justifyContent:"start"`、`alignItems:"start"`、`textAlign:"start"`。允许主信息组在剩余高度内垂直居中，但不得把垂直居中扩展为水平居中。Progress、环图、插画及按钮内部的局部居中不属于 V11，不受本条限制。
...
**2x4 单业务主焦点生成前置约束**：一个业务目标内存在一个被用户明确强调的主信息，以及一至两组属于同一目标的辅助信息时，优先使用 W1-focus-aux；左侧 `136×126vp` 主焦点区域固定不加 `backgroundColor`、`borderRadius` 或内容蒙版，右侧辅助区使用一个或两个蒙版。左、右区域只是同一业务的信息层级，不得按三个视觉区域误判为三个业务数据块或改走 W10。Progress 不是选择 W1 的条件；只有字段具有真实比例语义和可靠 `total` 时才可在左侧附加 Progress，没有合法比例时只展示主数字、主状态或主文字，禁止为了匹配骨架强行生成 Progress。
...
- **2x2 密度约束**：单业务且没有 CTA 时，主信息组优先在剩余高度内垂直居中或靠上，辅助字段紧贴主值；这里仅描述纵向位置，不改变默认水平左对齐。只有实际路由到 V11 或 V01 倒计时时整卡文字才居中；其他纯文字布局不得使用 `alignItems:"center"` 或 `textAlign:"center"` 模仿 V11。不得把标题、主值和辅助信息全部贴在顶部后留下无意义的中下部空白。存在 CTA 时，内容区使用 `layoutWeight` 吸收剩余空间，主信息组保持连续，CTA 沉底但不成为第一焦点。
...
1. `2x2` 和 `2x4` 的 root 可用高度都固定为 `126vp`，即 `150 - 12 - 12`。不得把 root 外部高度 `150vp` 当作内容高度。
...
5. `layoutWeight`、`flexShrink`、`clip` 和分布式对齐都不能抵消已经算出的固定高度。普通布局 `H_required > 126`、S4 `H_required > 134` 时必须删除弱区域、合并标题、降低区域固定高度或改选更简单骨架。

以下两种结构无论截图是否暂时可见都必须判定失败：

- `20 + 63 + 63 + 8 × 2 = 162 > 134`：标题行与 S4 纵堆双 `63vp` 区域不可共存；删除总标题，保留 `63 + 8 + 63 = 134`。
...
- `2x2` 最多使用 1 个内部内容背板；仅 S4 纵堆允许两个等高、同构、同层级的弱背板，并且此时 root 只能直接包含这两个区域，不再增加总标题或第三个区域。`2x4` 单数据块最多使用 1 个主内容背板和 1 个弱辅助背板；W9 固定使用两个 `138×134vp` 大内容蒙版，W10 固定使用一个 `138×134vp` 大内容蒙版和两个 `138×63vp` 小内容蒙版，W8 固定使用四个 `138×63vp` 小内容蒙版。W8/W9/W10 的 root padding 均固定为 8vp。列表项优先用间距、排版或 Divider 分组，不默认每项都套圆角底板。
- 小内容蒙版统一指 2x2 S4 的 `134×63vp` 分区和 2x4 W8/W10 的 `138×63vp` 分区，只允许两个单行 Text：第一行对象名与主数据使用 `14fp/700`、内容色 100%，第二行辅助数据固定 `12fp/400`、第十二节必要支撑文字色，不再增加独立业务标题行；两个 Text 都必须显式设置 `maxLines:1`。需要突出量化主值时，第一行优先展示主值，但同类对象分区必须允许“对象名＋主值”；对象名不得因数字优先而省略。蒙版内 1 项数据时只输出第一行，2 项时分别放第一、二行；有语义精确匹配且状态安全的候选素材时，1-2 项使用一个右侧图标，无合法素材时不得伪造或留空槽。2x2 S4 不得根据文字字符数、是否超过 6 个字、占一行还是两行来删除、移动或缩放右侧图标；使用固定文字列，静态文字放不下时先缩短文案并保持原语义，动态绑定则保持单行裁切，均不得换行形成第三行。2x4 W8/W10 必要对象名与主值放不下时仍可删除右侧视觉。3 项时不使用任何右侧视觉，第一行放主数据，第二行将两个辅助数据写成单个 Text 并以 ` | ` 分隔；超过 3 项时按“用户明确要求 > 主数据 > 状态 > 其他辅助”只保留前三项，其余删除。
- 小内容蒙版使用图标时固定为 `Row -> [text_column, visual]`，即使只有一行文字也必须用定宽 `text_column` 包装；父 Row 的 `itemMargin` 只能是 `8vp`，不得按通用组间距改成 10/12/16vp，也不得省略或写 0。visual 固定在右侧并垂直居中，右边缘距蒙版右边固定 12vp；`text_column` 的 1-2 行文字固定 `justifyContent:"center"`、`alignItems:"start"`，不得顶对齐。无 visual 的 Column 背板同样固定 `justifyContent:"center"`。独立 Image 固定 `20×20vp`：2x2 的文字列宽为 82vp，并满足 `12 + 82 + 8 + 20 + 12 = 134`；2x4 W8/W10 的文字列宽为 86vp，并满足 `12 + 86 + 8 + 20 + 12 = 138`。仅有 1-2 项信息，且目标字段通过 11.3 节 Progress 比例语义与可靠 `total` 检查时，visual 才可以是右侧环图；否则只能使用合法 Image 或删除 visual，严禁把任意数值包装成环。合法环按环外框右边缘距蒙版右边 12vp：2x2 S4 使用 `44×44vp` 环和 60vp 文字列，2x4 W8 使用 `40×40vp` 环；中心图标保持 20×20vp。3 项时必须删除包括 Image、Progress 和环中心内容在内的整个 visual；无合法图标且无合法环图时同样删除 visual，文字列占满蒙版内部宽度。
...
选择骨架前必须先按最终展示数据识别独立业务对象：同一语义实体下的名称、电量、状态等多个字段始终只算 1 个对象，`eventCandidates`、动作数量和组件数量均不得增加对象数；不同业务实体或同类业务中的不同对象实例（如两座城市）才分别计数。只有恰好 2 个独立对象时才允许 S4，且 root 必须使用 `padding:8` 并完整生成上下两个 `134×63vp` 分区；只有 1 个对象时必须选择 S1/S2/S3，绝对禁止生成单个 `134×63vp` 内容背板或其它残缺 S4。单对象加两个明确动作必须选择 S3，不得把该对象的字段拆成两个 S4 分区。
...
- 用于：一个业务对象、一个唯一主信息、恰好一个辅助字段且没有 action 的纯展示卡；固定参考 FEWSHOT_2x2 V11。辅助字段为零个或两个及以上时不得使用。倒计时继续固定使用 V01，不得借本亚型改变倒计时标题、单位或动作布局。
- 排他：只有 PromptBuilder 已选择 `centered-focus / 2x2-V11` 时才读取并执行本节；其它路由即使字段视觉上较少，也不得借用本节的居中标题、`focus_group`、中部大文字或底部居中辅助行。
- region：`root Column -> [title_area, focus_group]`。root `padding:12`、`itemMargin:8`、`alignItems:center`；`title_area` 固定 `126×28vp`，标题固定 `20fp/400` 且居中；`focus_group` 使用 `width:126`、`layoutWeight:1`、`justifyContent:start`、`alignItems:center`、`itemMargin:4`，直接子节点固定为 `[primary_area, supporting_text]`。`primary_area` 使用 `width:126`、`layoutWeight:1`、`justifyContent:center`、`alignItems:center` 将主信息固定在中间；`supporting_text` 固定 `126×20vp`、`flexShrink:0` 并沉底。
- 字号：主信息是纯 number/integer 时，数字固定 `38fp/700`，真实单位固定 `12fp/400`；主信息是字符串或带单位格式化值时固定 `24fp/700`。唯一辅助信息固定 `12fp/400`。标题、主信息和辅助信息均为单行居中。
...
### `W1-focus-aux`（单业务主焦点 + 辅助区域）

- 用于：单个业务目标具有一个明确主焦点，以及同一业务的一至两组辅助详情或动作；用户使用“主要、重点、最关心”等表达时优先。是否存在 Progress 不影响骨架选择。
- region：`content Row -> [focus_zone 136, aux_zone 130]` gap 10。`focus_zone` 固定 `136×126vp`，不设置 `backgroundColor`、`borderRadius` 或 padding，不形成蒙版；主读数默认 30fp、最大 38fp，主文字最大 18fp，辅助标签使用 12fp/400。存在真实比例语义和可靠 `total` 时，可在主信息下方增加 `Progress linear 136×8`（strokeWidth 8）及 12fp/400 端标行；否则让主信息组在左侧自然居中或按语义上下排列，不生成 Progress 或空进度槽。`aux_zone` 三变体：var-a 双份区域（两张 `130×59vp` 背板 + gap 8，白色 80% 底、圆角 12，各最多两行）；var-b 整体大区域（单背板 `130×126vp`、圆角 14，标题 400 顶部 + 正文 400 沉底、中部留白，无动作）；var-c 双按钮（两枚 `130×36vp` 胶囊纵排 gap 10，底部对齐）。
- 槽位：一个主焦点 + 同业务辅助 0-2 组；var-a 的两张辅助蒙版仍只算同一业务的两个辅助信息组，不增加业务数据块数量。
- 禁止：给左侧主焦点增加蒙版；因右侧存在两个蒙版而改判 W10；无比例语义时生成 Progress；辅助区混排背板和按钮；在 `focus_zone` 放入第二个独立业务。
...
- `20fp`、`24fp`：紧凑纯数字；`30fp`：主焦点纯数字默认档；`32fp`、`38fp`：内容少且空间充足时的数字放大档，数字最大 38fp。S1 `centered-focus` 是受控例外：标题固定 20fp，短文字主信息固定 24fp，纯数字主信息固定 38fp。
...
- 大字号数值同行的小字号 Text 只允许写真实单位，例如 `%`、`°C`、`天`、`小时`、`分钟`、`秒`、`步`、`次`、`件`、`个`、`km`、`mA`、`V`、`kcal`，或该数值字段 description 明确声明的其它单位；“最近安排”“当前状态”“后开始”等说明文字不是单位，必须移到主值下一行。除下方受控格式化主读数例外及 S1 `centered-focus` 的单个短文字主信息外，只有 number/integer 字段或纯数字静态值可以使用大于 `18fp` 的字号，名称、日期、时间、状态和格式化字符串即使放入名为 `value_num` 的组件也不能放大。
...
- 动态长名称、会议名、设备名和“可点击入口/对象名称”类内容（音乐入口、设置项、蓝牙设备名、联系人）不用 30fp 大字；改短静态主文案、放小字，默认 `fontSize:16`，内容少且空间充足时最大 `18fp` 并让 Text 占整行。只有 S1 `centered-focus` 的稳定标题允许使用 `20fp/400`，唯一短文字主信息允许使用 `24fp/700`，辅助文字仍为 12fp。
