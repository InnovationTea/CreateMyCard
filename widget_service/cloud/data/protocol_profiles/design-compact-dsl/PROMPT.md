...
- 会议时间线亚型：`2x2` 的单个会议或下一场会议固定参考 FEWSHOT_2x2 V06，使用黄色纯色时间线布局，不再使用会议融球信息列。
- 槽位：标题、两行信息、至多一个显式动作。
...
- action：有匹配事件时直接把 `onClick` 绑定到所属 zone，不生成独立 Button、ActionUnit、action_area 或动作文案；每区最多 1 个属于自身业务的 handler，两个业务各有动作时分别绑定各自 zone。同一事件不得重复绑定 root、另一区域或内部子组件。
