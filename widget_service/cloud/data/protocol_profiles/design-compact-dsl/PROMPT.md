...
**2x2 单业务多字段生成前置约束**：`/data` 下只有一个一级业务根时始终是单业务；同一根内的名称、状态及多个指标不得按字段数拆成 S4。禁止生成单个或残缺的 `134×63vp` S4 内容背板，使用全宽单业务信息流。用户明确要求展示且不用于动作参数的 1-3 个不同字段必须全部保留并各显示一次；“重点、优先、主要”只调整字段顺序和视觉主次，不得删除其余明确字段。重点字段放在前面。为适配 `150×150vp` 画布，普通 S1/S2 的全部信息区统一使用以下三类密度：①单一数字焦点含 `30fp/38fp` 大数字时，数字与真实单位合计为一行，其余字段最多组成一条 `12fp/400` 辅助信息行；若两个辅助事实需要分别阅读且纵向空间充足，主动把主读数降为合法的 `20fp/24fp` 格式化主读数或 `16fp/18fp` 普通主值，为两条辅助行释放高度，禁止为了保留大数字用 ` | ` 硬塞；②单一文字焦点最多一行突出文字和两行 `12fp/400` 辅助信息，短且自解释的唯一核心状态或结果满足第十节全宽、单行和压力预算时可使用 `20fp/24fp`；③两个及以上同级并行指标不得制造 `30fp/38fp` 大数字焦点，恰好两个短纯数字指标在各自宽度预算成立且共享单位明确时，可使用等宽双列和统一 `20fp`，每列保持“值＋标签”成组，否则改为对齐的纵向标签—值行；三个同级状态/等级/分类指标最多三行，标签列和值列分别对齐并使用统一普通字号，不从中任意挑选无标签 hero。视觉行按实际 Row/Column 结构计算，不按 Text 节点数量累加；Button、ActionUnit 和带 `onClick` 的图文动作 Row 都属于 action，不计入信息行。V01、V06、S3、S4、含 Progress 的视觉亚型及 2x4 W8/W10 小背板不适用本规则。2x4 W9 的左右两个 `138×134vp` 大背板必须分别把内部业务标题和 action 排除后独立执行同一密度规则，不能因位于 2x4 内而恢复额外信息行或多个突出焦点。普通单业务内容区不生成 Image，不得为使用候选素材套用 S4 小背板。

**2x2 单数值 Hero 安全盒前置约束**：仅当普通 S2 最终可见结构严格为“顶部标题＋中部唯一纯数字主值（可带真实单位）＋底部单动作”时启用。`content_area` 固定 `width:126`、`layoutWeight:1`、`justifyContent:"center"`、`alignItems:"center"`；其内放置独立的 `hero_box Column`，固定 `width:106`、`height:58`并双轴居中，`hero_box` 内只有一个 `value_row Row`，该 Row 固定 `width:106`、`justifyContent:"center"`、`alignItems:"bottom"`。数字和单位整组使用 `38/16fp -> 30/14fp -> 24/12fp -> 20/12fp` 的降档序列：先按 schema `sampleValue` 构造至少三位数字的压力值，再加 20% 宽度余量；宽度超过 106vp 或文字行高超过 58vp 就使用下一档，直至同时通过。不得用截断、省略单位、压缩容器或同时给 `content_area` 设置固定高度替代降档。中部一旦含有日期、时间、状态、说明、第二指标、Progress 或 Image，就不进入本结构，改按对应的普通密度、V08、并列指标或专用骨架处理。本规则不新增 few-shot，也不要从现有睡眠、环图或双指标示例拼装结构。

**单业务并行指标生成前置约束**：大数字 `value_row` 只能包含纯数字 Text 和紧邻的真实单位 Text，禁止在该 Row 内放入标签、方向、状态、名称、说明或其它字段；这些信息必须放在主值下一行。`value_row` 固定使用 `alignItems:"bottom"`，单位 Text 不设置与数字相同的固定高度，也不设置 `padding.bottom`；当前渲染器会按 Text 外框底边对齐，额外底部 padding 会把单位向上抬成视觉居中。主值绑定 string、名称或状态时不得生成相邻单位 Text，例如 `exerciseTypeName="户外跑步"` 后不能再追加“跑步”。先依据 userQuery 和字段 description 判断各指标是主辅关系还是同级关系，不得按字段顺序、文本长度或 sampleValue 大小任意选择 hero。同一业务对象内存在最高/最低、当前/目标、已用/剩余、左/右等同级量化指标时，禁止选中其中一项使用 `30fp/38fp` hero、数值与标签混合字号 Row 以及多个大数字 `value_row`。恰好两个短指标、每个指标的值和标签均能在独立槽内完整显示时，可使用一个等宽 Row 承载两个 Column；每列将值与自己的标签紧邻并居中对齐，两个值使用统一字号和字重，两个标签统一 `12fp/400`。值为短纯数字且共享单位能在标题或标签中明确时，两个值可统一使用 `20fp`；否则使用统一的 `14fp/16fp/18fp`。需要加强两列边界时，只增加一条低对比度竖向 Divider，不为两列各套独立背板。字段较长或压力预算不足时改为两条对齐的纵向“标签—值”行。恰好三个同级状态、等级或分类指标时，不从中制造无标签 hero；使用最多三条纵向标签—值行，标签列和值列分别共享对齐线，值统一 `14fp/16fp` 与字重，整组在剩余内容区垂直居中。日期、地点、来源、更新时间等元数据放到底部弱提示，不参与并列焦点组。此规则适用于 2x2 普通 S1/S2，也适用于 2x4 W9 的每个独立大背板；不适用于 V01、V06、S3、S4 和 2x4 W8/W10 小背板。
...
**2x4 双业务生成前置约束**：必须先按业务对象实例合并字段，再统计数据块；同一耳机对象的名称、连接状态、耳机仓电量和充电状态固定算一个数据块，禁止拆成“耳机连接”和“耳机仓电量”两个数据块。最终展示恰好两个业务数据块时先判断主次：存在一个明确主焦点、其余必要内容能压入两个辅助槽时锁定 W1-focus-aux；只有两个业务等权且都需要完整内容区时才锁定 W9。W9 root 必须是 Row、`padding:8`，直接且只能包含左右两个 `138×134vp` 大内容背板，间距 `8vp`；禁止复用 2x2 S4，禁止 `root Stack -> content Column`，禁止两个全宽背板上下排列，禁止任何其它上下双业务布局。禁止卡级公共标题、跨业务公共内容行、卡级 `actions/action_area` 和背板外按钮。每个业务的标题、数据和至多一个按钮必须全部放在自己的背板内，action 数量不增加、拆分或重排业务数据块，也不得为了容纳 action 改走 W10。W9 大背板带动作时直接子节点固定为 `[content, action]`，`content` 必须 `layoutWeight:1`，action 必须是最后一项并固定 `114×36vp`。纯文字 action 使用 Button；图文 action 使用 `Row -> [Image, Text]`，Row 必须显式写 `itemMargin:8`、左右 `padding:8`、`justifyContent:"center"`、`alignItems:"center"`，Image 固定 `20×20vp`。无动作的 `138×134vp` 倒计时大背板是例外：它直接且只能包含“目标标题、纯数字、单位天”三个 Text，背板固定 `justifyContent:"spaceBetween"`、`alignItems:"center"`，三个 Text 均使用 `width:114`、`textAlign:"center"`；禁止嵌套 content/readout Column 或增加第四行说明。其它 `content` 通常最多三行视觉信息；数字与单位拆成两个 Text 时仍属于同一行，多日天气允许使用“标题 + 三条日期摘要”的四行紧凑内容。辅助字段只有在两个短事实能够完整显示时才用 ` | ` 合并；三个以上动态字段不得塞进同一窄行，优先拆行，仍超出再删除低优先级可选字段。

**2x4 主焦点与同行读数前置约束**：W1-focus-aux 的电池诊断、手机与耳机充电、健康运动复盘等紧凑左侧信息组必须在 `focus_zone` 内水平、垂直居中，内层紧凑 Column 同样使用 `justifyContent:"center"` 与 `alignItems:"center"`；不得把主值或三行摘要贴在左上角。除倒计时背板固定的纵向“数字/天”外，任何 number/integer 与静态真实单位拆分显示时必须作为同一 Row 的相邻 Text，Row 使用 `alignItems:"bottom"` 和 `itemMargin:2`，数值与单位 Text 均使用内容自适应宽度，不设置固定 `width`；较小 Text 不设置 `padding.bottom`，禁止单位换到下一行。同一 Row 内其它不同字号 Text 也必须只依赖 Row 底对齐，不得用底部 padding 抬高小字号文字。天气与日程双业务同时展示当前温度和天气状态时，二者固定合并成一个单行 Text（如 `29° · 多云`），不得用不同字号的两个 Text 拼接造成可见基线不齐。
...
**同行文字可见字底校正前置约束（覆盖后文旧描述）**：2x2 与 2x4 的同一 Row 只要包含不同字号 Text，Row 固定 `alignItems:"bottom"`；每个较小 Text 的 `padding.bottom` 使用 `min(4, round((最大字号-自身字号)/2))`，最多只补偿 `4vp`。数字与单位 Row 另固定 `itemMargin:2`，数字和单位都使用内容自适应宽度。后文若仍出现“不同字号 Text 不设置 bottom padding”的旧描述，以本条为准。

**2x4 W1 内容节奏前置约束**：左侧信息组必须整体垂直居中；value-led、ring-focus 及只有一至两个短信息组的稀疏状态同时水平居中，事项列表和需要逐行阅读的长摘要只保持左对齐。value-led 最多形成“上下文 + 主焦点 + 支撑信息”三层，event/status/dense-summary 最多四个 Text，禁止五行以上内容挤入左侧。左侧和右侧辅助槽不得重复展示同一路径或同一语义事实，每个字段只分配到一个区域。
...
- 单业务卡只使用一个主显示组，以及一个单焦点或一个合法的并列焦点组；固定多对象骨架在各自内容蒙版内建立焦点。
- 一个主色族，最多一个状态色或动作色信号。
...
- 生成前先写出一句内部判断：“用户打开卡片首先要读到什么”。全卡只能有一个第一焦点，或一个由用户要求同级比较/概览而形成的并列焦点组；标题、单个主值、状态、按钮和并列焦点组不能同时争夺第一焦点。
- 主焦点按语义角色而不是字段顺序、字段名长度或 `sampleValue` 的视觉显眼程度排序：用户明确指定的重点与直接答案 > 整体结果、总量、主状态 > 组成项、局部指标、单次明细 > 日期、来源、更新时间。除非用户明确询问，局部子指标、缺省项或表示“没有/0项”的空结果不得压过可用的整体结果成为唯一大字号焦点；不能因为某字段更短就把它升级为 hero。
...
- **重心硬约束**：生成前先判断卡片属于“单焦点”还是“并列焦点组”。没有用户明确要求同级比较/概览、或字段语义不构成天然对照时，全卡默认只保留一个第一焦点；其它区域通过至少两项差异（字号、位置、连续留白、颜色明度、占用面积）降为支撑层。并列焦点组是一个整体焦点，组内保持同字号、同字重、同对齐和近似视觉重量，禁止再把其中一项单独放大。标题不能和主值同时使用最高强调，按钮不能通过更高饱和度抢过主值。
- **2x2 密度约束**：先按最终可见事实判断稀疏、标准或紧凑密度。稀疏单业务且没有 CTA 时，使用“顶部上下文＋中部主信息组＋可选底部元数据”三锚点，或把语义连续的主辅信息作为一个组垂直居中；`content_area` 默认写 `justifyContent:"center"`，只有信息组自身已占内容区约 70% 以上时才允许 `start`。不得把标题、主值和辅助信息全部贴在顶部后留下无意义的中下部空白。存在 CTA 时，内容区使用 `layoutWeight` 吸收剩余空间，主信息组在 CTA 上方垂直居中或上方居中，CTA 沉底但不成为第一焦点。若一个固定信息组占用不到可用高度的约 70%，不得继续使用顶部堆叠；改用居中、三锚点或适度放大唯一主焦点，不增加弱字段填空。
- **2x2 分行约束**：` | ` 只用于同层级、强相关、合并后仍完整通过单行压力预算且纵向确实受限的短信息。卡内仍有至少一行可用高度时，两个可独立理解的事实必须拆成两行；同一个 Text 引用两个及以上独立展示路径时，除固定小背板或明确受限的摘要槽外，优先判定为错误结构并改为两个 Text。不得为了维持 `maxLines:1` 把长句、标签和值全部横向挤在一行。一个事实本身需要换行时，只能按自然语义边界使用 `maxLines:2` 并显式预留两行高度，不能从数字、单位或固定词组中间断开。
...
- `assetCandidates` 只规定允许使用的素材范围，不得为了使用候选素材而新增内容区或图标槽。标准 2x2 单业务 S2 已经具有 CardHeader 图标槽时不属于“新增图标槽”：只要候选 `description` 与整卡主题精确匹配、状态中性且标题宽度预算成立，就默认使用一枚右上角主题图标。对 `asset.*.png` 彩色位图，只要其 `description` 明确匹配当前单一主业务并允许主题图使用即可保留；不再以透明背景作为前置排除条件，最多使用一枚并保留原色，优先放标准 2x2 S2 CardHeader 现有右上 20vp 位。2x4、S1/S3/S4、正文、按钮、环中心和融球仍不主动使用该类 PNG。
...
- `2x2` 在选择骨架和分配空间前，先保留用户明确指定部分，再按允许的图标位置、数量上限和区域互斥确定图标。对标准单业务 S2，必须执行一次 CardHeader 图标机会检查：从候选中寻找与整卡主题精确匹配的中性业务/对象/指标图标；找到且标题压力预算成立时将其归为 `shouldKeep` 并默认保留，只有发生明确冲突时才删除。仅用户明确要求本身超过 2 个时允许超额，不再增加其他可选图标；没有语义准确且状态安全的候选素材时不得伪造。
- 用户要求展示某个字段或提供某个操作，不等于明确要求为其配图标，也不允许超出图标总量或使用不准确素材；但标准单业务 S2 的现有 CardHeader 槽位命中精确、状态中性的主题素材时，按上一条默认保留，不再以“用户没有明确要求图标”为由删除。
- `2x2` 内容主视觉、环中心和 S4 分区视觉继续执行区域互斥；CardHeader 主题图标与按钮动作图标职责不同，在总量不超过 2、语义不重复且各自宽度预算成立时允许共存。若必须取舍，用户明确指定图标优先，其次保留回答核心问题的内容/分区视觉，再保留状态中性的 CardHeader 主题图标，最后才是可省略的按钮装饰图标。符合条件的彩色 PNG 优先放入 CardHeader 右上现有 20vp 位，不改变骨架、不新增字段、不调整图标大小。无图标标题不写 CardHeader.icon，无图标动作从布局前采用纯文字并保留动作，不生成 icon-round、图文按钮或空图标槽。
...
生成前必须分别审查每个数据字段、事件和素材，并在内部归类；`2x2` 素材只按 2.5 节允许位置、数量和区域互斥裁决，不能因对应数据或动作是 mustKeep 就把配图同步列为 mustKeep。标准单业务 S2 的精确、状态中性 CardHeader 主题图标按 2.5 节归为 `shouldKeep` 并默认保留：
...
三类候选独立裁决，不要求数量对齐。数据和素材没有最低使用数量，标准单业务 S2 的精确 CardHeader 主题图标与 8.3 节小内容蒙版的 1-2 项信息图标规则除外；显式动作遵循 `mustKeep`，隐式入口遵循 `shouldKeep`，副作用动作没有显式用户意图时直接归入 `drop`。画布放不下时，按以下顺序缩减：`drop` 候选 → 纯装饰素材 → 普通 `shouldKeep` 字段 → 精确且状态中性的 CardHeader 主题图标 → 隐式入口；不得删除用户明确要求的显式动作，也不得通过裁切、超小字号、压缩点击热区或堆叠所有候选解决容量冲突。
...
5. **图标匹配职责**：CardHeader 图标表达整卡主题；分区图标或环心图标表达该对象/指标；按钮图标表达该按钮实际动作。不得把按钮的图标借给内容标题，不得用其中一个业务图标代表多业务整卡。先读素材 description，再核对可见文字、绑定字段和所属对象；路径名称仅辅助判断，不能覆盖描述。单业务 S2 存在准确的中性主题素材且 CardHeader 标题预算成立时，默认使用该标题图标，而不是继续输出纯文字标题。
...
7. **准确则用，不确定则不配图**：标准单业务 S2 的 CardHeader 候选只要描述足以支持整卡主题、状态中性且标题宽度成立，就默认使用；没有描述足以支持当前用途的素材时不显示图标，不留空槽，不为了彩色、左右对称或满足数量配错图。“有合法素材”始终指当前候选中语义精确匹配且状态安全的素材。缺少动作图标时保留纯文字动作，不能丢失事件或改用无关图标。
...
1. 本提示词中的协议硬规则。
2. TaskSpec 声明的数据、事件、素材和尺寸上限；标准单业务 S2 的精确中性 CardHeader 主题图标及 8.3 节小内容蒙版的 1-2 项信息图标按对应规则默认保留，其它候选存在不构成必须使用要求；事件按显式动作、隐式入口和副作用动作分级处理。
...
- 原始数值与单位使用多个 Text 做视觉分层时，除 2x2 V01 单目标倒计时和 2x4 W9 倒计时背板外，数值 Text 与承载单位的静态 Text 必须是同一 Row 中按顺序相邻的节点；紧邻大数字的 Text 只能写声明单位本身。说明文字必须放在主值下一行。2x2 V01 与 2x4 W9 倒计时背板固定使用纵向数字/单位结构，整组只能出现一个单位“天”，禁止再生成行内单位。
...
**2x4 白色内容背板颜色（覆盖后文旧的 80% 描述）**：仅 `2x4` 卡片中的白色内容背板、内容蒙版和可点击内容背板统一使用 `#99FFFFFF`（白色 60%）；不得继续使用 `#CCFFFFFF`。`2x2` 保持现状，其中 S4 白色内容背板仍使用 `#CCFFFFFF`（白色 80%）。本规则只约束背板的 `backgroundColor`，不改变白色文字、图标、按钮或融球的透明度。
...
- 普通单业务：root padding 12，CardHeader 高 20，组间距 4；y=36vp 只是内容区起点，不是主信息必须贴顶的视觉锚点。先区分“必要解释”和“独立支撑”：必要解释紧跟主读数并共同居中，真正的日期、更新时间、范围或来源等独立支撑才可沉底。
- 有底部胶囊按钮：content_area 用 layoutWeight:1 接收剩余空间，内部“主值＋描述”作为连续信息组；一至两行稀疏信息固定使用 `justifyContent:"center"`，三行且高度预算紧张时才允许 `start`。action_area 为 root 最后一项，高 36，固定沉底。内容很少时优先放大唯一主焦点或增加组内自然行距，不把所有文字贴在 y=36vp。
- 无底部胶囊按钮：稀疏内容固定使用“CardHeader 顶部＋主信息组居中＋独立支撑沉底”；`content_area` 内一至三行且总高度不足其可用高度约 70% 时必须使用 `justifyContent:"center"`。没有真正独立的底部信息时不强建 footer，而是让完整主信息组垂直居中。标准或紧凑内容可从内容区上方开始，但不得留下比信息组更显眼的整块下方空白。
...
2x4 每张卡必须且只能选择一个骨架，允许在声明范围内微调，不得跨骨架拼接 region。先判断主次关系：一个明确主焦点加最多两个辅助槽优先使用 W1；没有明确主次时，再按数据块数量选择 W2-W10，2 个等权数据块使用 W9，3 个等权数据块使用 W10，4 个等权数据块使用 W8。通用约束：独立卡级标题只允许用于单数据块骨架，有则必须使用 20vp 高的 CardHeader 作为全尺寸前景 Column 首项，无则不生成标题及空槽；W8/W9/W10 严禁卡级 title、CardHeader、公共标题行及标题空槽，并统一使用 root `padding:8`；W1 及其他普通单业务骨架使用 `padding:12`，W8/W9/W10 多业务蒙版统一宽 138vp；线性进度条一律 `strokeWidth:8`；弹性沉底用包含真实内容的 `layoutWeight:1` 容器占据剩余空间（`flexShrink` 只收缩不拉伸，禁止用于沉底）；禁止 `justifyContent:"spaceBetween"` 制造三段均分（会把中部元素推离预期位置）。

### `W1-focus-aux`（左主焦点 + 右双辅助区域）

- 用于：存在一个明确主焦点，且其余必要字段、状态或动作可以压入右侧两个辅助槽。可承载单业务，也可承载“主业务 + 一个弱辅助业务”；Progress 不是选择条件。
- region：root Row 使用 `padding:12`、`itemMargin:10`，直接包含左侧 `focus_zone 136×126vp` 与右侧 `aux_column 130×126vp`。focus_zone 不加白色内容背板；aux_column 固定上下两个 `130×59vp` 背板、gap 8、白色 60% 底、圆角 12。
- focus_zone 只选一种内部变体：① value-focus：一个 `30fp/38fp` 纯数字和小单位，最多两条辅助信息；② ring-focus：一个有真实比例语义的环形 Progress 和紧邻状态；③ event-focus：最多三项事项，每项标题与时间/地点压成连续信息组；④ status-focus：一条最大 20fp 的突出状态和最多两条辅助信息。不得同时制造两个 hero。
- 左侧纵向节奏：`136×126vp` 不得把所有内容挤在顶部后留下大块无意义空白。value-focus/ring-focus 的三层信息按“顶部 12fp 上下文 + 中部 `layoutWeight:1` 主焦点并垂直居中 + 底部 12fp 支撑信息”组织，并保持左对齐。纯文字 status-focus 只有 2-4 行短文本时，全部放进同一个无 `layoutWeight` 的紧凑 Column，由 focus_zone 使用 `justifyContent:"center"` 让整组垂直居中；三行以上仍左对齐，不把第一行固定在顶部。最终可见内容只有一个主信息组、最多再加一条短辅助信息时，使用稀疏居中变体：focus_zone 与内容组都设置 `alignItems:"center"`、主值 Row 设置 `justifyContent:"center"`，相关 Text 设置 `textAlign:"center"`；数字和单位仍视为同一个主信息组。事项列表、长提醒正文和真正需要多行阅读的摘要继续左对齐。禁止仅因字段原始数量少就居中，必须按最终可见语义组判断；不使用 `spaceBetween` 生硬拉开。
...
- 右侧每个辅助背板只承载一项指标、一组紧密相关的两行状态摘要或一个动作，最多两行文字、两个动态事实和一个可选 `20×20vp` 图标；左右耳机等天然成对信息允许用两行分别承载左右对象，每行最多两个动态事实。一个普通槽保留两个动态事实时必须分别使用两个 `maxLines:1` Text，每行各自包含完整的“短标签 + 值/状态”，不得把两个标签放第一行、两个值放第二行，也不得用 `|` 合并成一行后依赖 clip；三个以上事实先删除最低优先级项。只有一个有效状态时只保留一行，禁止以静态文案重复动态状态（例如两行都显示“未充电”），未被用户要求的 updatedAt 不用于填满槽位。带图标时固定使用 `Row -> [78vp 文字 Column, 20×20vp Image]`，Row 左右 `padding:12`、`itemMargin:8`、`justifyContent:"start"`、`alignItems:"center"`，文字 Column 垂直居中、`alignItems:"start"` 且只含一至两个 `maxLines:1` Text；Image 必须是最后一个直接子节点，固定在右侧。动作图标同样放在标签右侧，禁止 `Image -> Text`、图标在文字下方或依赖 clip 露出背板。无图标时文字区使用 106vp 宽度，Row 使用 `justifyContent:"start"`，Column 使用 `alignItems:"start"`；所有 Text 固定 `textAlign:"start"`，只允许内容组垂直居中，禁止动作文字水平居中。动作直接绑定整个背板，视觉规格仍为 `130×59vp`，不得在卡片底部另加满宽 CTA，也不得留下空辅助背板。
...
- 单业务选择：一个主状态或主读数已经能回答核心问题，并且还有 2 个可归并的辅助组时，优先使用 W1。尤其是“4 项以上事实 + 显式动作”、成对设备状态、天气预警与生活指数、单日程加提醒/导航等场景，不再使用通栏文字流和满宽底部按钮。
- 数据归属：左侧可显示主业务根；右侧每个槽分别引用一个所属对象的数据或事件。不同根不能混进同一槽，动作参数与槽内数据必须同属一个对象。动作占辅助槽但不增加数据块。
- 双业务主次：两个业务的有效字段量相差至少两项、且弱业务只有 1-2 项时，按一个主焦点加弱辅助业务处理，使用 W1 而不是给弱业务分配空旷的 W9 大背板。若弱业务已有两项完整内容且两个业务各有一个明确动作，仍保留 W9，避免丢失业务入口；弱业务只有单项时仍使用 W1。若两个右侧槽都被明确动作占用，可在左侧用最多三行普通字号摘要统一回答同一个用户目标，但不得制造第二个 hero。
- 禁止：左侧白色蒙版、三列并列指标、全宽底部按钮、右侧超过两个背板、因存在多个 `/data` 根自动改成 W9。只有两个业务确实等权且都需要完整内容区时才使用 W9。
...
- region：可选 `CardHeader 20vp` + `body 98`：`progressSlot 45`（主数值行 20fp/700 + label 12fp/400 + `Progress linear 276×8 strokeWidth 8`）+ `details Row 45`（两背板 134×2 gap 8、圆角 10、白色 60% 底，各「标题 12fp/400 + 值 12fp/400」）。
- 槽位：一个进度主指标 + 两个详情项。
- 禁止：进度条细于 8vp、三背板、详情行高 >50。

### `W6-agenda-cta`（左侧日程焦点 + 右侧双入口）

- 用于：下一日程/单事件 + 两个真实入口。
- region：复用 W1 的横向几何，root Row `padding:12`、`itemMargin:10`；左侧 `136×126vp` 无背板区域承载日程标题、时间和一条必要上下文，右侧上下两个 `130×59vp` 白色 60% 背板分别承载一个动作并绑定各自 `onClick`。
...
- 用于：近期日程/待办 3 行清单。
- region：可选 `CardHeader 20vp` + `list 102`：三行背板 276×28（gap 8）、圆角 8、白色 60% 底；每行文本 12fp/400，不附加装饰图标。
- 槽位：恰好三行；每行一条文本。
- 禁止：两行或四行、行内按钮、行高不一。
...
- region：无卡级标题；root `padding:8`，使用 `284×134vp` 的 `2×2` 网格，两行两列的横纵 itemMargin 均为 8，每个小内容蒙版固定 `138×63vp`、padding 12、borderRadius 16、白色 60% 背景，并完整遵守 8.3 节小内容蒙版的两行数据、右侧图标和数据删减规则。仅在蒙版内有 1-2 项信息且需要比例视觉时允许使用右侧 `40×40vp、strokeWidth:4` 的环形 Progress 与环心 20×20vp 图标；3 项信息时不得保留环图，否则不得为填充版面强加环图。
...
- 无 visual 的 `138×63vp` 小内容蒙版必须使用 Column，把一至两个 Text 作为纵向连续信息组垂直居中；禁止用 Row 将两个 Text 横向摊开。只有存在右侧 Image、环或其它合法 visual 时才使用“左文字、右 visual”的 Row。
...
- region：无卡级总标题；`root Row` 以 `width/height:matchParent` 填满 `300×150vp` 画布，padding 8、itemMargin 8，直接包含左右两个 `138×134vp` 同级背板。两背板均为 Column、padding 12、borderRadius 16、白色 60% 背板，内部可用空间均为 `114×110vp`。这是 2x4 双业务唯一合法的一级布局，不得套用 2x2 S4 的上下两个 `134×63vp` 结构，也不得改成上下两个全宽背板。
- 每侧按 2x2 的层级组织并独立选择内容变体；需要突出量化主值时按「对象名 + 主读数 + 必要辅助信息 + 可选底部按钮 114×36vp」排列；有必要对象名时先说明对象，再突出主值；没有突出数字时仍可按「业务标题 12fp/400 + 主内容 + 可选底部按钮」排列。有按钮时主内容使用剩余高度，每侧最多三行视觉信息。业务标题只是背板内分区标题，不使用 CardHeader。倒计时与另一个独立业务组成 W9 且倒计时侧无动作时，倒计时背板直接且只能包含三个 Text，依次为目标标题、`30fp/38fp` 纯数字、唯一一个 `12fp` 单位“天”；背板固定 `justifyContent:"spaceBetween"`、`alignItems:"center"`，三个 Text 均全宽居中，禁止嵌套内容 Column 和第四行辅助文字。数字不得再包含“天”，不得压成 `14fp` 的同行 `30天`，也不得引用另一背板的数据补充倒计时内容。
- 每个 W9 背板按 `150×150vp` 的 2x2 内容密度独立检查：内部 `12fp/400` 业务标题和底部 action 不计入信息行；使用 `30fp/38fp` 纯数字主值时，除主值/单位行外最多一条 `12fp/400` 辅助信息；纯文字或格式化主读数最多一条突出信息和两条 `12fp/400` 辅助信息；同级并行数据最多三条普通字号完整信息，不得制造多个突出焦点。普通窄行最多合并两个确实能完整显示的短事实；三个以上动态字段禁止串入同一个 Text。仍有行高时拆成独立 `12fp` 行，空间不足再删除最低优先级字段，不得移动到另一背板或背板外。多日天气的单日摘要可按下条规则合并更多同日字段。
- 稀疏分区：用户最先强调的业务承担全卡第一焦点，其背板内容组使用 `justifyContent:"center"`，主值优先使用安全的 20/24/30fp 档位，核心状态优先使用 18fp；另一背板使用 16/18fp 主信息和 12fp 必要支撑信息，不得镜像复制相同的标题、主值、辅助行模板。某侧最终只保留 1-2 项必要事实且无底部按钮时，有语义精确且状态安全的候选素材可把 `20×20vp` 业务图标放在标题右侧或主内容组右侧，文字组与图标间距固定 8vp；内容已达 3-4 行、图标会挤压必要文字或没有精确候选时保持纯文字，不留空槽。左右通过不同的信息层级形成主次，同时以内容组位置和连续留白保持视觉重量平衡。
- 多日天气在 W9 内每一天只能使用一个 `12fp/400` Text。同一天同时提供完整日期和星期时优先保留短星期并删除完整日期；随后按“星期、天气、温度、降雨”顺序压入，例如 `周四 · 多云 · 25~32℃`，后续字段只有经宽度预算确认完整可见时才追加。禁止将一天拆成三行、缩小字号或依赖裁切，禁止在日期之间插入 Divider。三天天气可使用“业务标题 + 三条天气”的四行内容预算。
- 多日天气行必须按 schema sampleValue 做 `114vp` 压力预算，温度范围必须完整显示，禁止输出只剩 `25°C/` 的残缺范围。`星期/周几 + 天气 + 温度范围` 超宽时优先压成 `周几 + 温度范围`，再超宽则只保留完整温度范围；降雨概率只在整行仍能完整显示时追加。美观和完整值优先于把所有字段硬塞进一行。
- action：每侧至多一个属于该业务的动作入口。默认使用可见 Button 或图文按钮 Row，并只把事件绑定该按钮；当 userQuery 明确说“点击卡片/点击天气区”等整块点击且没有要求可见按钮文案时，允许把唯一事件直接绑定所属背板，视为该侧已保留动作，不再重复生成按钮。root 和非动作内容不得绑定 `onClick`。只有一侧有动作时，另一侧不生成按钮或空槽；不得生成卡级 action_area。先固定左右业务数据块再放 action，禁止为按钮新增数据块或改成 W10；动作参数绑定的数据根必须与所在背板显示的数据根一致，例如引用 `/data/weather/` 的天气动作只能放进天气背板，不能放进倒计时背板。音乐动作不得放进天气背板，未被用户明确要求且无法归属当前业务的动作必须删除。
- 按钮空间：背板内动作固定 `114×36vp` 且标签保持完整单行。信息与动作竞争高度时，先删除最低优先级辅助字段，再缩短业务标题；天气详情动作优先使用“查看天气”而不是“查看天气详情”。禁止压缩按钮高度、把按钮推出背板或让动作标签出现省略号。
- W9 动作文案同样只保留一个完整命令，禁止在按钮或图文动作内追加“点击打开”“进入设置”等重复提示。数值单位本身不能说明业务含义时，必须带短标签，例如“风力 2级”，不得只显示裸露的“2级”。
...
- 禁止：上下双业务布局、`root Stack -> content Column -> [zone_1, zone_2]`、两个全宽 `276×59vp` 背板、卡级总标题、主辅不对等分栏、背板外动作、整卡点击、没有明确整块点击意图时绑定背板、左右业务共用按钮、第三业务。
...
- region：无卡级标题；`root Row` 以 `width/height:"matchParent"` 填满 `300×150vp` 画布，padding 8、itemMargin 8，直接包含左侧 `138×134vp` 大内容蒙版和右侧 `138×134vp` Column。右侧 Column 使用 itemMargin 8，上下各放一个 `138×63vp` 小内容蒙版。三个内容蒙版均使用 padding 12、borderRadius 16、白色 60% 背景。
...
- 融球或用户指定的场景背景只配简单骨架和极少背板；2x4 浅色微渐变 root 的内容蒙版使用白色 60%，仅 W8/W9/W10 按固定骨架使用 4/2/3 块内容蒙版，其他骨架不复制多个迷你卡片。
...
- `14fp`：内容标题、状态、正文、CTA。
- `16fp`：主焦点文字默认档；`18fp`：普通主焦点文字放大档。2x2 稀疏单业务卡中，短且自解释的唯一核心状态或结果可以使用 `20fp/24fp`，但必须是全宽单行、显式 `maxLines:1`、通过压力预算且父容器无横向 padding；对象名、日期、时间、来源、更新时间和普通说明仍最大 18fp。
...
**格式化主读数例外（与转换前校验一致）**：适用于明确描述为温度、时长或百分比的主读数，不限于单一 data 根；名称、日期、时间、状态不适用。内容可以是直接 PathBinding，也可以是只引用一个字段并追加真实单位的简单 Expression，不得拼接名称、状态或说明。Text 必须单行、无 padding/margin、显式高度至少为字号的 1.4 倍，并与实际主内容槽同宽：2x2 使用 126vp，2x4 全宽使用 276vp，W9/W10 左侧 `138×134vp` 大背板使用内部 114vp；父级 Column/Row 必须同宽且无 padding。只允许 20fp 或 24fp；各槽默认使用 20fp，只有按最长合法值加 20% 余量完成压力预算后才可使用 24fp。W8、小内容蒙版与环中心不使用该例外；不能通过增大 Text 声明宽度突破父级预算。

**短核心状态例外（与转换前校验一致）**：只适用于 2x2 稀疏单业务卡的唯一第一焦点，例如简短天气现象、设备状态或结果词；不得用于对象名、标题、日期、时间、来源、更新时间或普通说明。Text 只能使用 20fp 或 24fp，必须全宽单行、`maxLines:1`、无 padding/margin，父级为同宽且无 padding 的 Column 或单子节点 Row，并按最长合法值加 20% 余量检查。核心状态超过一行或不能自解释时退回 16fp/18fp，并通过自然分行建立层级；不得用 30fp/38fp 放大文字。
...
5. 空间不足时优先把次要状态移到主值下方、扩大主值槽位或降低到批准字号；纵向仍有空间时优先分行，不要把多个独立事实继续压进一个 Row。不要从已经格式化且自带单位的动态字符串中剥离单位另造静态 Text；原始 number/integer 字段需要按描述补充静态单位时，允许按 4.2.1 拆分数值与包含单位的自然后置文案。
...
- 数字与单位拆成同一 Row 内的两个 Text 时，不论数字使用 14/18/20/30/38fp 中的哪一档，Row 都必须使用 `alignItems:"bottom"` 和 `itemMargin:2`；数值与单位 Text 均不设置固定 `width`，让二者按实际内容紧邻排列。`value_suffix` 或 `value_unit` 字号 12-16，不设置 `padding.bottom`，不设置与数字相同的固定高度，单位不能用 30 号字。同一 Row 内其它不同字号 Text 同样只使用 Row 底对齐；当前渲染器中固定文字槽会拉开数值与单位，底部 padding 会把小字号文字向上抬成视觉居中。补充说明使用相同或更弱的字号、字重和颜色，不得争夺主数值焦点。
...
- W9 的 `114vp` 天气内容区同时展示温度范围、降雨概率和空气质量时，每项各占一个 `12fp` 单行 Text，使用“温度/降雨/空气”短标签，不加图标、不把两项挤进同一行。三行之外不再追加日期、地点或重复天气标题，必须保证“空气质量”等标签完整可见。
...
- 普通图文按钮使用 `Row + Image + Text + onClick`，Row 是完整的按钮视觉与点击外框，不是普通内容行：高度默认 `36vp`、圆角 `18vp`、左右 padding 至少 `8vp`、`itemMargin: 8`、内部内容居中；内部 Image 默认 `20×20vp`，Text 使用 `14fp/400-500`。W1/W6 的 `130×59vp` 右侧动作背板是明确例外，执行对应骨架的“文字在左、Image 在右”结构，不得套用普通按钮的 `Image + Text` 顺序。若按钮外框窄于父容器内部宽度且设计意图为水平居中，还必须按第 8.2 节为按钮外框建立显式的父级居中约束，不能把内部内容居中当作按钮外框居中。
...
- 浅色内容背板按尺寸区分：2x2 使用 `#CCFFFFFF`（白色 80%），2x4 使用 `#99FFFFFF`（白色 60%），包括 S4、W8/W9/W10 及其他骨架已声明的内容背板；白色内容背板沿用 UX 分区规范，不误用本图内容色 20% 替代；不为增加层次额外创建背板。背板内沿用本卡文字色，不再套一层有色背板，不叠加描边和阴影。
...
- 时区、无关更新时间、重复状态默认不展示；用户明确要求 boolean 状态时，必须使用条件表达式映射成“已连接/未连接”“开启/关闭”“是/否”等符合字段语义的用户可读文案，任何可见 Text 都禁止直接绑定 boolean 而显示 `true/false`。任何取舍都不能删除用户核心需求。
...
1. 提取唯一服务对象、主问题和用户明确要求；按“直接答案 > 整体结果/总量/主状态 > 局部指标 > metadata”确定唯一第一焦点，再裁决数据与素材的 `mustKeep/shouldKeep/drop`，并按 2.4 节给事件标注 `explicit/implicit/sideEffect` 后再映射优先级；2x2 选择骨架前按 2.5 节允许位置、数量和区域互斥确定图标，标准单业务 S2 必须执行 CardHeader 图标机会检查并默认保留准确的状态中性主题图标，内容区只保留环中心或 S4 双业务主视觉图标，不为文字信息行配图；2x4 沿用原素材选择规则；两个尺寸最终选择的所有图标均固定 20×20vp。
2. 严格采用 TaskSpec.size，按第九节路由到一个固定骨架。若 `mustKeep` 无法映射，先删除 `shouldKeep` 并回退同尺寸更简单骨架，不跨骨架拼接。
3. 为骨架填入角色槽位，确定共同对齐线、主辅面积和留白；只保留最小充分数据、事件和素材。对 2x2 单业务先估算最终信息组高度：不足可用高度约 70% 时改用居中、三锚点或适度放大唯一主焦点；纵向仍有一行空间时把独立事实分行，不用 ` | ` 横向硬塞。显式动作必须落到合法 CTA，S4 及 W8/W10 小内容蒙版除外并直接绑定所属蒙版，W10 左侧大内容蒙版仍使用内部 Button；隐式入口默认作为不抢占空间的 root 入口，S4 及 W8/W10 小内容蒙版改绑所属蒙版；未被显式要求的副作用动作必须删除。
...
4. **骨架与预算**：是否保留每个并列对象的名称和有歧义指标的标签；必要文字是否使用主文字色而非 60%；普通 S2 是否按密度选择居中、上方居中或三锚点，一至两行稀疏 content_area 是否明确使用 `justifyContent:"center"`，描述是否紧跟对应主值、动作沉底，且没有全部贴顶后留下大块下方空白；是否只使用一个固定骨架；单业务双按钮是否为 S3 的 40vp 信息组加 78vp 动作组，主信息是否合并为 14fp 且没有独立标题和大数字内容区；是否按最终展示对象正确统计数据块，2x4 的 1 个数据块是否只在满足明确前提时使用 W1-W7、否则使用已有全宽单列内容流，2/3/4 个数据块是否分别路由到 W9/W10/W8，2x2 恰好两个数据块是否强制使用完整双分区 S4，单数据块是否未生成单个 `134×63vp` 内容背板或残缺 S4，且没有把任一数据块降级为另一数据块的辅助信息；2x2 S4 两个背板内的文字、可染色图标、Progress 和 Divider 是否统一使用同一主内容色 RGB；所有小内容蒙版是否只有两行、左文字右 visual，1-2 项如有右侧图标，是否语义准确且状态安全，3 项时是否去掉 visual 并以 ` | ` 合并辅助数据，超过 3 项是否删除低优先级项，图标或环外框是否距蒙版右边 12vp；CardHeader 是否只用于 2x2 S2 或 2x4 单数据块的独立卡级标题、每卡最多一个且位于规定前景 Column 首项，W8/W9/W10 是否没有卡级标题、CardHeader、公共标题行或标题空槽，无标题布局是否未留空槽，S1/S3/S4 是否未加 CardHeader 或右上角辅助图标；root 宽高是否为 `"matchParent"`、普通单业务布局 padding 12、S4 及 W8/W9/W10 padding 8、圆角 20、clip true；root Column 是否使用 `alignItems:"center"`，root Row 的固定参考宽度直接内容是否使用 `justifyContent:"center"`，且没有把 root 的整组居中误写成内部文字全部居中；是否已按 3.1 节逐项写出 root 直接子节点的最小高度、margin 和 `itemMargin` 并确认普通布局 `H_required <= 126vp`、S4 及 W8/W9/W10 `H_required <= 134vp`；分布式对齐是否只分配扣除最小间距后的非负剩余空间；所有 Row/Column 两轴预算是否非负，动态文字 Row 是否保留余量，点击热区是否至少 24vp。
5. **文字与图表**：卡片级标题是否为稳定的业务、对象或事项主题而非日期、时间、时长、倒计时或其它业务值，日程/会议/提醒缺少事项主题时是否使用默认标题“日程”而非省略，是否默认不超过 8 个字符，超长例外是否已证明完整可读且不挤压其它内容；受保护文本和 CTA 是否完整；同一动态字段或同一事实是否只由一个可见 Text 展示，格式化主值是否未重复拆出单位，独立辅助字段是否未被误作主值单位；是否没有空白 Text、`textOverflow`、单独的 `°` 或近似温度单位；格式化值是否包含单位与符号并通过压力检查；需要突出的量化主值是否位于所属业务主内容区第一行，环形 Progress 是否作为主内容第一项，日期、时间和编号是否未被误判为主数字；纯数字是否默认 30fp 且不超过 38fp，普通文字是否不超过 18fp，短核心状态或格式化主读数是否只在受控例外中放大到 20/24fp，单位是否为 12-16fp，卡片标题和按钮是否分别为 12fp/14fp；纵向有空间时独立事实是否已经分行，` | ` 是否仅用于确实受限的同级短信息；全卡字号是否不超过三档、同层级元素是否保持一致；每个 Progress 是否具有 TaskSpec 明确声明的比例语义和可靠 `total`，且没有为温度等普通数值生成 Progress 或编造 `total`。
6. **表面与素材**：逐个核对 Image 的实际用途，2x2 普通内容区必须为零，例外仅为环中心或 S4 分区主视觉；标准单业务 S2 是否已执行 CardHeader 图标机会检查，存在精确、状态中性且标题预算成立的主题候选时是否默认使用，省略时是否确有标题压力、状态风险、用户禁用或主视觉冲突；S4 每区最多 1 个，两个天气对象有语义准确且状态安全的候选素材时是否各有 1 个，重复 src 是否分别计数，辅助文字前后是否无图标；W9 稀疏分区有精确候选素材时是否优先使用右侧业务图标，密集分区是否未为对称强塞图标；是否按最终图标实例计数、默认不超过 2 个且按 2.5 节预先分配；CardHeader 主题图标与不同职责的动作图标是否只在总量和宽度预算成立时共存；2x2/2x4 的所有图标是否均为 20×20vp 且没有主辅尺寸分档；是否仅为用户明确要求放宽数量，且未丢失动作或预留空图标槽；未有用户明确要求时，是否只使用蓝、紫、暖三套固定浅色微渐变或五套融球；业务映射和融球优先级是否正确；文字与单色图标、按钮、背板是否遵循本卡固定配色和透明度；进度与分隔线是否只更新颜色基准而保留原样式；是否保留多色图标与真实状态色例外，且没有旧的同色背板、特殊深色或自由取色残留。
7. **最终简化**：是否只有一个主焦点、清晰对齐线和有限表面；第一焦点是否按用户重点与整体语义选出，且没有被局部子指标或空结果错误取代；并列分区的高度、视觉重量和留白是否均衡；两个以上独立事实是否在纵向有空间时分行而不是用 ` | ` 合并；是否已删除弱装饰、重复事实、无关字段、假交互、无意义单子容器和多余材质；若仍有任何不确定布局，是否已经回退到同尺寸更简单骨架。
