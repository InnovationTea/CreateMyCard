...
**2x4 主焦点与同行读数前置约束**：W1-focus-aux 的电池诊断、手机与耳机充电、健康运动复盘等紧凑左侧信息组必须在 `focus_zone` 内水平、垂直居中，内层紧凑 Column 同样使用 `justifyContent:"center"` 与 `alignItems:"center"`；不得把主值或三行摘要贴在左上角。除倒计时背板固定的纵向“数字/天”外，任何 number/integer 与静态真实单位拆分显示时必须作为同一 Row 的相邻 Text，Row 使用 `alignItems:"bottom"` 和 `itemMargin:2`，数值与单位 Text 均使用内容自适应宽度，不设置固定 `width`；较小 Text 按字号差使用 `min(8, ceil((最大字号-自身字号)/4))` 的 `padding.bottom`，禁止单位换到下一行。同一 Row 内其它不同字号 Text 也使用相同补偿规则。天气与日程双业务同时展示当前温度和天气状态时，二者固定合并成一个单行 Text（如 `29° · 多云`），不得用不同字号的两个 Text 拼接造成可见基线不齐。
...
**同行文字可见字底校正前置约束（覆盖后文旧描述）**：2x2 与 2x4 的同一 Row 只要包含不同字号 Text，Row 固定 `alignItems:"bottom"`；每个较小 Text 的 `padding.bottom` 使用 `min(8, ceil((最大字号-自身字号)/4))`，最多补偿 `8vp`。数字与单位 Row 另固定 `itemMargin:2`，数字和单位都使用内容自适应宽度。后文若仍出现“不同字号 Text 不设置 bottom padding”的旧描述，以本条为准。
