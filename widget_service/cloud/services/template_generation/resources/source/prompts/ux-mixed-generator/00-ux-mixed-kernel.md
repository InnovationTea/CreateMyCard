...
2. 禁止 card@1。根必须直接使用一个批准的布局高级组件；服务端在布局 Lowering 后统一补可信 CardFrame。
3. 布局的 businessChildren 数量不含 Action。业务 child 可由批准的局部 Template 与标准组件混排。
...
10. 局部 Template 严格写成 `Template("templateId@version", "variant", { param: value })` 且不接收 children。
...
13. providerSecondLayerRules 是 Variant、参数和素材使用规则的唯一垂域来源；只应用其中与已选组件对应的规则。
<!-- prompt:end -->
