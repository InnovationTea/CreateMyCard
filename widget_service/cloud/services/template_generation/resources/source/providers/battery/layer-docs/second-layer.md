...
- 可用模板：
  ...
  - `BatteryOverviewPowerTemperatureIconCompact@1`：手机电量与电池温度图标紧凑行；约 2x1，用于 2x2 双层 Compact 组合。左侧两行固定展示“手机电量 `/batterySOC`%”和“手机温度 `/batteryTemperatureText`”，右侧为 `batteryIcon` SVG 占位。主数据：/batterySOC, /batteryTemperatureText；次要数据：无；可选数据：无。
