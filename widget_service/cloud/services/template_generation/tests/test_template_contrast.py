...
对比度豁免门禁保留为普通测试，未转场景金样：``template_root`` 标记
抑制 VISUAL.CONTRAST 的语义依赖「改名单后低对比内容必须重新被检出」
的正反双向判定，全量预览豁免扫描同时是 113 个预览用例的计数钉——
两者价值都在布尔不变量本身，冻结快照不增加评审信息；产物本体已由
Layer A 模板金样覆盖。引擎或校验器改动后按 golden 工作流复核。
"""
