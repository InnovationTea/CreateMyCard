...
- 不扩展目录外的公共协议和服务入口；公共校验边界包括第 7 节的模板对比度豁免，以及第 8 节
  既有单位校验的节点类型和单位归属修正。本地 HTTP 调试补丁、凭据、画廊和安装包不属于提交范围。
...
单位检查仍属于 `semantic`，不随模板 `quality` 豁免而跳过，也不通过删除正确单位、改写业务标题
或放行校验失败来适配画廊。后置单位扫描只处理连续的静态 Text，遇到容器、图片、进度组件、
缺失节点、非字符串内容或另一个动态字段时停止，不把非文本组件缺少 `content` 当成业务数据缺失。

同一 Row 内保留“数值 + 天后开始”等相邻单位说明；Column 中只把独立的纯单位 Text 归属于前面的
数值，不将下一行辅助标题中的单位字样计入。例如“剩余30天”下面的“倒计时 + 天气组合画廊”
不是重复单位。独立单位沿用注册的单位及别名；真正缺少单位、重复追加单位和已带单位字段旁的
多余单位仍报错。其它容器中的并列节点不构成后置单位组。

回归必须同时覆盖非文本兄弟节点、独立辅助标题、合法同行说明、纵向纯单位、真实缺失和重复单位，
并通过完整生成入口复测受影响模板，不能以模板编译通过替代最终单位校验。
画廊样例覆盖只能写入该模板实际投影的字段，预处理错误必须保留具体阶段和原始原因，
详见 [端到端画廊](provider-template-e2e-gallery.md)。
