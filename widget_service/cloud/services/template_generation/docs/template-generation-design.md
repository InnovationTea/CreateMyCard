...
在线生成另有来源驱动的 Compact 专项豁免：模板生成器成功返回后，服务端为本轮输出设置模板来源，
公共 Compact 处理器跳过 `validate_compact_dsl()`，但保留格式往返转换、必要解析、绑定处理、单位后处理、
素材映射和启用状态下的 A2UI Artifact 校验。模板内部编译和两次修复不变；普通模型回退、外层模型修复、
编辑的新输出执行完整 Compact 校验。组件标记不授予该来源豁免，下文主文字标记规则仍用于直接调用校验 API。

来源仅在当前请求内存中保留，不新增本地或 OBS 来源存储。历史编辑保持完整 Compact 校验，
不区分原始生成来源；历史源和编辑新输出都可能被专项规则拦截。模板新建成功不保证后续可编辑。
Compact 高度、密度、染色等专项规则与模板编译非等价，需回归正式模板样例；各入口失败策略、
公共模板 quality 与缺失单位豁免均不扩大。
