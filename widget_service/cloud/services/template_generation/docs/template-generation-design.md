...
公共 Compact 解析会先检查普通 children 树的可达性；运行时 IF 的分支属性不构成树边，
因此孤立分支会提前失败。最终 A2UI 校验也拒绝未知组件；模板内部在模板解析、Tersel 解析及
Compact 归档转换阶段拒绝运行时 IF。不为测试兼容另行改写目录外的公共入口。
...
也不因被跳过的检查触发质量修复。
正式模板来源由服务设置 `skip_compact_dsl_validation=True`，跳过整个 Compact 校验 API。
直接调用该 API 时，`_collect_hero_value_errors` 才按有效模板标记跳过主文字检查；
两种入口不能混同。公共解析、转换和最终 Artifact 校验仍执行。
同一有效模板根标记还用于跳过 `semantic` 阶段的 `DISPLAY_UNIT_MISSING`，允许数值与可见单位
分属不同的固定布局容器。融球、非融球和独立模板预览均适用；普通卡片和无效标记仍检查单位缺失。
`DISPLAY_UNIT_DUPLICATED`、其它最终 semantic 和 hard 继续执行。正式模板来源不执行转换前
高度预算和首帧检查；标准 A2UI 阶段没有等价高度检查，首帧数据根缺失仅告警。
Compact 解析先去重，原始同 ID 冲突无法由后续模板根检查追溯；直接 A2UI 的重复 ID 仍检查。
完整边界见 [模板后处理与校验说明](post-processing-validation.md)，运行时 IF 仍不支持。
