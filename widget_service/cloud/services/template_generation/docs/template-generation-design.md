...
11 个现有业务新增 128 个 Number/Text/Pair × Full/Hero/Compact/Support 通用模板，以参数化显示槽覆盖固定字段组合之外
的场景。Search 按业务优先保留专用候选，仅零命中业务开放通用模板；Planner 沿用原布局与动作逻辑，
不能在专用候选无法成案后再兜底。第二层只新增所选通用模板的 data 填充权限，完整方案仍由服务端锁定。
完整语法、类型、数组边界、动态路径和覆盖校验见 [Provider 模板接入约定](provider-template-contract.md)。

验证覆盖专用优先、缺失能力、单/双业务布局、数组 0/1/2 项、混合纯值与动态绑定、表达式类型、越界与
跨业务路径拒绝、最终显示字段完整性及 Support 事件归属。单独记录自动化编译、预览、真实模型和真机
验证状态，不能将演示预览或模拟模型调用计为真实模型效果。
