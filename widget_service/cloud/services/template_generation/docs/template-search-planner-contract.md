...
每个业务先只检索专用模板；只要该业务还有任何完整覆盖显式字段的专用候选，就排除全部
`fallbackOnly` 通用模板。没有专用候选时，才按已批准的单能力根、真实输出 Schema、TaskSpec 中的
具体字段类型和卡片尺寸准入通用候选。禁用的 Provider/Template 不得重新进入候选。
此决策在 Planner 之前完成：已有专用候选却无法组成布局时仍按原契约退出，不重新开放通用模板。

通用候选再按 `generalContentKind` 与字段语义匹配：数值及单一单位量使用 Number，状态、叙述、
日期使用 Text，组合时长按所属业务字号使用 Number，两个以上字段可选择 Pair。显式主字段存在时用于约束族选择，并进入 Plan 的
`primaryMatchedFields`；同一健康能力下各业务由 `generalDataPaths` 限定允许引用的字段范围。
多个 Plan 必须和最高排序方案保持相同业务集合和主题，不能因能力字段重叠而混入另一套业务作用域。
...
通用模板额外携带 `dataSchema`、`dataSources`、`dataRules`：第二层在调用参数对象的 `data` 字段中填写
纯数据、`$path("/relative/path")` 或受限 `Expr(...)`，其余字段仍是原有 Props。数据来源只投影当前
Plan 的显式字段、类型、语义描述和单位，不传 TaskSpec 全量内容或样例值。普通模板不开放 `data`。
`contentKind` 和 `requiredMainFields` 说明展示语义和主值必须引用的字段；二层不得将主字段降为辅助文本。
...
通用模板还校验每个数据参数的真实类型、数组长度、能力根和引用路径；表达式必须静态证明所有结果
分支符合声明类型。最终可见 Text 必须引用该 Plan 要求的全部动态字段，放进未显示参数、样式或动作
的引用不算覆盖。纯文本不能伪装为动态绑定。通用日程 Support 沿用同一展示项的动作归属判断，使用
Plan 锁定的显式路径识别事件索引；跨事件组合不允许绑定单个事件动作。
Number 主值另做数值格式与字段语义校验，禁止状态词或标签句子；Number/Text 的主值或 Pair 的两个值
必须引用指定主字段。可选进度只允许百分比/睡眠评分或 0–100 的纯数值，不能套用在温度、时长等量上。
