...
天气 Support 的状态演示值只覆盖目标模板实际声明并投影的天气现象字段：当前天气使用
`/current/condition`，后日预报使用 `/daily/2/condition`，其它已登记预报索引按模板字段保持原样。
不得仅按 Support 后缀给所有天气模板注入当前天气字段，也不得为演示值额外创建未请求的字段。
输入回归逐项核对所有样例覆盖路径均在该用例的候选字段中，并覆盖预报数组中的实际下标。
受信样例覆盖非法、模板注册表加载失败、数据预处理失败分别记录阶段和原始原因，继续沿用现有
生成失败处理，不把所有预处理错误统一描述为“模板注册表不可用”，也不改为静默忽略非法覆盖。
