...
| 天气温度/紫外线 Support | 不传图标 | 多云样例没有对应注册资源；保留状态文字，不用温度计或太阳替代 |

电量、日程、倒计时和无图标心率 Support 保持原有纯文字结构，不增设图标槽位。
应用时长、系统内存仍因数据能力未注册保留缺失场景；应用品牌和内存槽位也不能借用其他业务资源。
以上是测试素材选择，不是生产素材全集。生产按当前候选描述、槽位语义以及实际状态匹配，
天气状态未知或没有对应图标时省略；不读取样例值生成运行时组件分支。
...
以 2026-09-06 当前资源为基线，应生成 8 个业务分组、1 个跨业务组合和 1 个双业务段落分组，共 115 个用例；
其中新增 45 个 Support 配对用例。无模型 dry-run 中 14 个状态为 `missing`，101 个状态为 `not_generated`。
...
- `--provider gallery.two-support`：只批跑双业务段落，覆盖全部 15 种 Support 模板的 45 个场景。
...
当前基线：自动化 115 个场景 / 101 份 A2UI；显示画廊 85 个场景 / 75 份 A2UI / 10 个缺失占位。
其中双业务段落由 45 个自动化场景缩减为 15 张显示卡（13 成功、2 缺失）。每份入选 A2UI 应与源文件
