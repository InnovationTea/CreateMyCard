...
紫外线 Full 将 20vp 指标值与 14vp 指标说明纵向排列，空气质量 Hero 使用 20vp 主值；
心率 IconCompact 为图标标题行保留 20vp 高度，平均心率值使用 18vp。上述调整不改变模板 ID、输入字段、
数据绑定或事件配置，画廊仍从模板源重新编译，不直接修改生成后的 A2UI。
