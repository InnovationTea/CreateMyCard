...
- 首次将 PR 的有效树差异 squash 到最新主线时，未改动原 PR 分支，也未合并主线；
  用户随后要求回同步 PR256，处理方式见“回同步 PR256”。
...
在新分支集成完成后，用户追加要求将冲突解决结果同步回 PR256。

- 保留原 PR 头提交 `5d19efba` 的历史，通过普通合并接入集成提交 `86e96713`；
  推送目标为 `yanshuifeng/CreateMyCard:codex/2x2-multi-business-planner`，不强推、不合并 main。
- 回同步中的 13 个冲突文件复用上节已经验证的解法。解决后、补充本节前，
  Git 树哈希与集成提交完全相同：`3d246ee6b6f11b4ad2a62ff8e83c00068ecc51dd`。
  本轮仅额外更新本文记录，不另外改动源码、配置或模板。
- 相比原 PR 头提交，合并共涉及下列 32 个文件，其中包含继承自 main 的变化。
  相比最新 main，PR 的净差异仍为模板子系统内的 75 个文件，完整清单见下一节。
- 同步范围内 16 个 Python 文件 Ruff、CardPlan Bundle 一致性检查通过。
- 本轮完整模板测试加主线对比度专项回归：1214 通过、5 失败，耗时 111.29 秒。
  其中模板子系统 1189 通过、5 失败，对比度专项 25 项全通过；失败项与上节已有失败完全一致。
  命令为 `python -m pytest cloud/services/template_generation/tests tests/test_contrast_validator.py -q --tb=short`，
  仍使用上节环境变量；差异空白检查通过。
- 未执行真实模型画廊生成或端侧安装；旧工作区未跟踪的临时文档不纳入提交。

### 本次合并文件及用途

以下为相对原 PR 头提交的全部文件，路径相对于仓库根目录：

- [AGENTS.md](../../../../../AGENTS.md)：继承主线新增的校验字段记录入口，未重写协作规范。
- [docs/对比度校验方案演进.md](../../../../../docs/对比度校验方案演进.md)：继承主线对比度规则演进说明，不新增模板方案到根目录。
- [resources/base/media/drop_1.svg](../../../../../resources/base/media/drop_1.svg)：继承主线水滴素材样式。
- [widget_service/cloud/data/capabilities/app-11.7.5.205_rom-6.0/asset_capabilities.json](../../../../../widget_service/cloud/data/capabilities/app-11.7.5.205_rom-6.0/asset_capabilities.json)：继承主线 205/6.0 素材描述更新。
- [widget_service/cloud/data/capabilities/app-11.7.7.300_rom-7.0/asset_capabilities.json](../../../../../widget_service/cloud/data/capabilities/app-11.7.7.300_rom-7.0/asset_capabilities.json)：继承主线 300/7.0 素材描述更新。
- [widget_service/cloud/services/card_validation/contrast_validator.py](../../../../../widget_service/cloud/services/card_validation/contrast_validator.py)：继承主线渐变与融球场景的对比度校验修复。
- [widget_service/cloud/services/card_validation/字段约束与演进记录.md](../../../../../widget_service/cloud/services/card_validation/字段约束与演进记录.md)：继承主线字段约束记录。
- [widget_service/cloud/services/template_generation/docs/pr256-main-integration.md](../../../../../widget_service/cloud/services/template_generation/docs/pr256-main-integration.md)：记录本次基线、冲突处理、验证结果及完整提交文件清单。
- [widget_service/cloud/services/template_generation/docs/provider-template-contract.md](../../../../../widget_service/cloud/services/template_generation/docs/provider-template-contract.md)：同步可选字段变换、Support 和图标颜色契约，保留主线多绑定契约。
- [widget_service/cloud/services/template_generation/docs/provider-template-e2e-gallery.md](../../../../../widget_service/cloud/services/template_generation/docs/provider-template-e2e-gallery.md)：更新组合画廊规则、交付流程及集成后的 129 项输入规模。
- [widget_service/cloud/services/template_generation/docs/provider-template-preview-gallery.md](../../../../../widget_service/cloud/services/template_generation/docs/provider-template-preview-gallery.md)：更新原子预览规格与集成后的 105 个业务模板计数。
- [widget_service/cloud/services/template_generation/engine/advanced/content_selectors.py](../../../../../widget_service/cloud/services/template_generation/engine/advanced/content_selectors.py)：继承主线对多组天气输入的内容提取支持。
- [widget_service/cloud/services/template_generation/engine/cardplan/compiler.py](../../../../../widget_service/cloud/services/template_generation/engine/cardplan/compiler.py)：支持绑定计划验证与显式图标颜色策略，保留主线安全边距及多绑定编译。
- [widget_service/cloud/services/template_generation/engine/cardplan/fusion_ball_background.py](../../../../../widget_service/cloud/services/template_generation/engine/cardplan/fusion_ball_background.py)：继承主线天气内容安全边距处理。
- [widget_service/cloud/services/template_generation/engine/cardplan/models.py](../../../../../widget_service/cloud/services/template_generation/engine/cardplan/models.py)：增加完整 Plan 数据模型并保留主线 binding_count。
- [widget_service/cloud/services/template_generation/engine/cardplan/provider_bundle.py](../../../../../widget_service/cloud/services/template_generation/engine/cardplan/provider_bundle.py)：支持 present 可选字段与有值变换、事件及资源验证，保留主线多绑定。
- [widget_service/cloud/services/template_generation/engine/cardplan/retrieval_index.py](../../../../../widget_service/cloud/services/template_generation/engine/cardplan/retrieval_index.py)：将 optionalData 纳入字段覆盖索引。
- [widget_service/cloud/services/template_generation/engine/cardplan/template_retrieval.py](../../../../../widget_service/cloud/services/template_generation/engine/cardplan/template_retrieval.py)：实现数据职责单一的 Search，并适配主线多根数据绑定。
- [widget_service/cloud/services/template_generation/engine/pipeline.py](../../../../../widget_service/cloud/services/template_generation/engine/pipeline.py)：串接第一层、Search、Planner、第二层与计划验证。
- [widget_service/cloud/services/template_generation/resources/source/providers/weather/layer-docs/first-layer.md](../../../../../widget_service/cloud/services/template_generation/resources/source/providers/weather/layer-docs/first-layer.md)：继承主线新增天气字段的一层意图说明。
- [widget_service/cloud/services/template_generation/resources/source/providers/weather/layer-docs/second-layer.md](../../../../../widget_service/cloud/services/template_generation/resources/source/providers/weather/layer-docs/second-layer.md)：同步天气第二层模板使用、资源和业务动作约束。
- [widget_service/cloud/services/template_generation/resources/source/providers/weather/provider.json](../../../../../widget_service/cloud/services/template_generation/resources/source/providers/weather/provider.json)：同步天气模板注册、字段要求、参数或资源/动作约束。
- [widget_service/cloud/services/template_generation/resources/source/providers/weather/templates/weather-overview.cardtpl](../../../../../widget_service/cloud/services/template_generation/resources/source/providers/weather/templates/weather-overview.cardtpl)：迁移天气正式模板的布局、文本、绑定、图标与交互调整。
- [widget_service/cloud/services/template_generation/resources/source/themes/family-weather-care-blue/theme.json](../../../../../widget_service/cloud/services/template_generation/resources/source/themes/family-weather-care-blue/theme.json)：继承主线关怀天气主题配置。
- [widget_service/cloud/services/template_generation/resources/source/themes/fusion-weather-blue/theme.json](../../../../../widget_service/cloud/services/template_generation/resources/source/themes/fusion-weather-blue/theme.json)：继承主线融球天气主题配置。
- [widget_service/cloud/services/template_generation/tests/test_provider_gallery_batch.py](../../../../../widget_service/cloud/services/template_generation/tests/test_provider_gallery_batch.py)：验证组合输入、资源、动作及 129 项画廊计数。
- [widget_service/cloud/services/template_generation/tests/test_template_contrast.py](../../../../../widget_service/cloud/services/template_generation/tests/test_template_contrast.py)：将对比度预览覆盖计数同步为 105。
- [widget_service/cloud/services/template_generation/tests/test_template_generation.py](../../../../../widget_service/cloud/services/template_generation/tests/test_template_generation.py)：同步综合链路回归，并适配主线双城/关怀天气用例的一层输出格式。
- [widget_service/cloud/services/template_generation/tests/test_template_plan_planner.py](../../../../../widget_service/cloud/services/template_generation/tests/test_template_plan_planner.py)：验证 Plan 原子性、主焦点和字段覆盖，新增双根完整/缺字段/错类型 3 项回归。
- [widget_service/cloud/services/template_generation/tests/test_template_preview_dataset.py](../../../../../widget_service/cloud/services/template_generation/tests/test_template_preview_dataset.py)：验证 105 个业务模板及各尺寸、布局预览计数。
- [widget_service/cloud/services/template_generation/tests/test_template_retrieval.py](../../../../../widget_service/cloud/services/template_generation/tests/test_template_retrieval.py)：同步检索职责、模板字段覆盖与可用性回归。
- [widget_service/tests/test_contrast_validator.py](../../../../../widget_service/tests/test_contrast_validator.py)：继承并复跑主线对比度规则回归。
## 完整提交文件清单
