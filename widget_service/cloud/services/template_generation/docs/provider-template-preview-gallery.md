...
数据集覆盖 82 个业务模板，不包含 10 个布局模板：

- Compact：30 个，使用 2×2 卡片容器，模板内容高度 68vp。
- Hero：4 个，使用 2×2 卡片容器，模板内容高度 124vp。
- Full：32 个，使用 2×2 卡片容器，模板内容高度 136vp。
- WideHero：1 个，使用 4×2 卡片容器，模板内容高度 124vp。
- WideFull：15 个，使用 4×2 卡片容器，模板内容高度 136vp。
...
- `T001.json` 至 `T082.json`：每个模板独立的 A2UI 消息数组，固定包含 `createSurface`、
