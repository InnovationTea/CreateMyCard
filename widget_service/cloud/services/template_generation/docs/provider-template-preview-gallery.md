...
数据集覆盖 74 个业务模板，不包含 7 个布局模板和 2 个 Action 模板：
...
- WideHero：6 个，使用 4×2 卡片容器，模板内容高度 124vp。
- WideFull：10 个，使用 4×2 卡片容器，模板内容高度 136vp。
...
- `T001.json` 至 `T074.json`：每个模板独立的 A2UI 消息数组，固定包含 `createSurface`、
