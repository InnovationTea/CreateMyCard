...
- `day_area` 优先展示会议日期或日期标签，必须保留为 root 第一个直接子节点的
  `136×16vp Row`，固定左对齐且内部只有一个 Text；禁止把日期 Text 直接挂在
  `alignItems:"center"` 的 root 下。除 V01 单目标倒计时外，顶部标题和日期上下文
  一律左对齐。`meeting_texts` 固定为会议标题加最多两行辅助信息，会议标题固定
  `14fp/700`，辅助信息固定 `12fp/400`。
