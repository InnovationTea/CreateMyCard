...
class JsxA2UIBridge:
    ...
    def __init__(
        ...
        *,
        ...
        self.options = options or BridgeOptions(
            ...
            thinking_mode=(
                "high" if self.settings.deepseek_enable_thinking else "disable"
            ),
        )
        self.dump_to_workspace = dump_to_workspace
    ...
    async def generate(self, task_spec: object, size: object) -> BridgeResult:
        ...
        logger.info(
            f"{_MODULE} agent_start component={component_name} size={size}"
        )
        ...
        logger.debug(
            f"{_MODULE} === RAW JSX === component={component_name}\n{raw_jsx}"
        )
        logger.debug(
            f"{_MODULE} === RAW A2UI ({len(raw_a2ui)} messages) === component={component_name}\n{raw_a2ui_json}"
        )

        # 落盘 raw jsx/a2ui/trace/rejected/context 到 workspace，供 runner 抢取
        # 端到端运行时关闭（dump_to_workspace=False），与模板方案/默认方案一致只保留 artifact
        if self.dump_to_workspace:
            self._dump_raw_to_workspace(component_name, raw_jsx, raw_a2ui, raw_source, prepared.prompt_task)
            self._dump_trace_to_workspace(
                component_name, trace_data, result, prepared.prompt_task, prepared.compile_context,
            )
    ...
    def _dump_raw_to_workspace(
        ...
        """把 agent 原始 JSX、A2UI 写到 workspace 目录，供 runner 轮询抢取。"""
        try:
            _WORKSPACE_DIR.mkdir(parents=True, exist_ok=True)
            jsx_path = _WORKSPACE_DIR / f"raw_jsx_{component_name}.jsx"
            a2ui_path = _WORKSPACE_DIR / f"raw_a2ui_{component_name}.json"
            ...
            logger.info(
                f"{_MODULE} raw_dumped component={component_name} "
                f"jsx={jsx_path} a2ui={a2ui_path}"
            )
    ...
    def _dump_trace_to_workspace(
        ...
        """把 trace、rejected JSX、context 写到 workspace 目录，供 runner 轮询抢取。"""
        try:
            _WORKSPACE_DIR.mkdir(parents=True, exist_ok=True)
            ...
            trace_entry = {
                ...
                "semantic_status": result.get("semantic_status", "completed"),
            }
            trace_path = _WORKSPACE_DIR / f"raw_trace_{component_name}.json"
            ...
            for turn_record in turn_trace:
                ...
                rejected_path = _WORKSPACE_DIR / f"raw_rejected_{component_name}_turn{turn_num:02d}.jsx"
            ...
            if compile_context and (compile_context.get("data") or compile_context.get("actions")):
                context_path = _WORKSPACE_DIR / f"raw_context_{component_name}.json"
            ...
            logger.info(
                ...
                f"trace={trace_path} rejected={rejected_count} context={'Y' if compile_context else 'N'}"
            )
        except Exception as exc:
    ...
    def _default_request_context(self) -> ModelRequestContext:
        ...
        return ModelRequestContext(
            ...
            device_id=device_id,
            country_code=(
                country_code or self.settings.deepseek_platform_default_country_code
            ),
