...
| 占位属性 | 2×4 规格 |
|---|---|
| `width` | 占满 276vp 父模块；不提供自由 size Prop |
...
- 外层模块提供完整 276vp 内容宽度，禁止通过 `style`、`className` 或额外 width Prop 改变规格。
...
### 4.6 NumericRatioStack

恰好三个同级占比值需要紧凑排列时使用。组件内部复用 `NumericRatio` 的单项结构，并由 `direction` 决定横排或纵排；一项或两项不得使用。
...
| 占位属性 | 值 | 说明 |
|---|---|---|
| `height` | 横排 16vp；纵排 56vp | 三个单项各占 16vp，纵排使用两处 4vp 间距 |
| `width` | 横排为父槽宽度；纵排为 `max-content` | 横排根据父槽宽度自动压缩内部间距并均匀分布，三个百分比可在 118vp 内容槽内完整显示 |

## 3. 按钮组件

### 5.3 CardButton

卡片按钮，由必选文本、可选资源 Icon（缺省时显示圆形占位 Icon）和按钮容器组成，仅用于 300 × 150vp（2×4）Card 的固定操作槽。组件本身不声明固定宽高，而是占满父容器分配的槽位。
...
- 只能用于当前尺寸 Card。
- 用于“左内容右侧双槽／左侧双槽右内容”和“四槽宫格”的固定 Action 槽；单个下方 Action 仍使用 `CardButton`。
- 竖排固定槽列使用“左内容右侧双槽／左侧双槽右内容”；“四槽宫格”的四槽必须填满，允许与 `InfoBlock` 分列混用。“左右双区”子布局中的按钮使用 `PillButton`，不使用 `CardButton`。
- 组件使用 `width: 100%`、`height: 100%` 占满父槽。宽高由外层 `Stack` 或 `Grid` 按 Layout Pattern 分配，禁止给组件传固定尺寸。
- 每个父槽固定为 132 × 57vp；禁止使用 276vp 整卡宽操作槽。
- 父槽必须满足“宽度 ≥ 高度”；不得把 CardButton 放入窄高槽位。
- 多个 `CardButton` 不得仅做一行左右并排；“四槽宫格”为完整的两列两行结构，`CardButton` 与 `InfoBlock` 可按业务关系自由分配槽位，不生成三槽网格。
- `CardButton` 必须完整位于 132 × 57vp 的固定父槽内；竖排或网格中的按钮槽应通过父 `Stack`／`Grid` 明确分配，不使用 `alignSelf`。
