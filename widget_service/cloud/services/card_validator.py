...
class CardValidationReport:
    ...
    def passed(self, strict: bool = False) -> bool:
        ...
        入参：
        - strict：是否把 warnings 也视为失败。
        出参：通过返回 True，否则返回 False。
        """
...
def validate_card(
    ...
    if isinstance(cardspec, str):
        card_spec_value = load_cardspec(cardspec, reporter)
    else:
        card_spec_value = load_cardspec(
            json.dumps(cardspec, ensure_ascii=False),
            reporter,
        )
    if len(messages) >= 3:
    ...
    if strict and reporter.warnings:
        ...
        return CardValidationReport(
            errors=reporter.errors + reporter.warnings,
            warnings=reporter.warnings,
        )
    return CardValidationReport(errors=reporter.errors, warnings=reporter.warnings)
...
def main() -> int:
    ...
    report = validate_card(genui_text, cardspec_text, strict=args.strict)
