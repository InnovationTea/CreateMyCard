...
**Project Root**: repository root containing `Automation/`, `ArkTs/`, `visual_aesthetics/`, and `queries.jsonl`
...
3. **DevEco CLI** (`devecocli`) available when HarmonyOS docs/tooling are needed
4. **HDC (HarmonyOS Device Connector)** in system PATH
5. **Test device(s)** connected via USB debugging, or emulator running
6. **Doubao API access** (Volcano Ark endpoint + API key)
...
4. Verify DevEco CLI when using HarmonyOS helper tooling:
`powershell
devecocli --version
`
...
skills/                  # Project-local Codex skills approved for this repository
`
...
- Project-local `skills/gitpush` is approved for committing and pushing fork branches after the user asks to push.
- Project-local `skills/gitsync` is approved for syncing a fork mainline from `upstream` to `origin` after the user asks to sync.
- DevEco CLI was owner-approved for this HarmonyOS/ArkTS project; verify with `devecocli --version` before relying on it.
- Superpowers is owner-approved as a recommended Codex workflow plugin, but it was not automatically installed in this environment. Install it through the Codex plugin UI when available, then verify it appears in available skills/plugins.
- `git-code-tracker` was explicitly declined for this project.
- Knowledge asset write mode is stored in `.agents/agent-seed.json` and is currently `full-access`, so future agents may update `AGENTS.md` and `agents.d/` directly within owner-confirmed onboarding/update scope.

### agents.d Index
- `agents.d/bootstrap.md`: setup checks, environment variables, first useful run.
- `agents.d/tooling.md`: approved CLIs, project-local skills, external plugin recommendations, safety levels.
- `agents.d/development-loop.md`: daily verification loop and blocked checks.
- `agents.d/architecture-map.md`: data flow, module boundaries, files that change together.
- `agents.d/debug-playbook.md`: reusable symptoms, diagnostics, recovery steps.
- `agents.d/change-recipes.md`: common change workflows and required checks.
- `agents.d/review-handoff.md`: evidence to report before review.
- `agents.d/risk-areas.md`: high-risk workflows and escalation triggers.
