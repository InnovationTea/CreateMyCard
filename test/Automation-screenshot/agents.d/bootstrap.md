...
5. Confirm DevEco CLI when HarmonyOS helper tooling is needed:

```powershell
devecocli --version
```

Success signal: prints a version. Observed after owner-approved install: `1.0.0`.

Do not run `devecocli init` or MCP/project-mutating setup commands without separate owner approval.

6. Confirm query cases exist:
