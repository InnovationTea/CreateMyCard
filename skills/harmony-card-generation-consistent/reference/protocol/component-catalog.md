...
- 动态展示值按 [`data-binding.md`](data-binding.md)；字符串拼接按 [`function.md`](function.md)；root shell、事件边界、禁用能力和 JSONL 消息顺序按 [`protocol.md`](protocol.md)。
...
- 容器只有 `Row`、`Column`、`List`、`Stack`；普通 `children` 只写组件 id 字符串数组。
- 模板循环对象只给 `Row`、`Column`、`List.children`，形态固定为 `{ "componentId": "...", "path": "/items" }`；`Stack.children` 不使用模板循环。
- 展示值优先字面量、`{"path":"/..."}` 或 `formatString`；新卡片不要把示例里的表达式当默认写法。
...
- `Progress`：进度条或进度环；必需 `total`；`value` 为数字、`{"path":"/..."}` 或遗留表达式受限例外；`total` 同规则且必选；`styles.type` 取 `linear|ring|eclipse|scaleRing|capsule`；`styles.color` 为颜色字符串。
- `Button`：语义按钮；必需 `label`；使用 `label` 和 `onClick`，不用 `Button.action`；`label` 为字符串、`{"path":"/..."}`、`formatString` 或遗留表达式受限例外；`enabled` 为 boolean、`{"path":"/..."}` 或遗留表达式受限例外；`onClick` 为 EventHandler 数组；`styles.fontWeight` 为数字或 `normal|regular|medium|bold|bolder`。
- `Checkbox`：多选框，只在用户明确要求切换状态时使用；无固定额外必需字段；`label` 为字符串、`{"path":"/..."}`、`formatString` 或遗留表达式受限例外；`value` / `group` 为字符串、`{"path":"/..."}` 或遗留表达式受限例外；`select` 为 boolean、`{"path":"/..."}` 或遗留表达式受限例外；`styles.selectedColor` / `styles.unSelectedColor` 为颜色字符串；`styles.shape` 取 `circle|rounded_square`；`styles.mark` 为 `{ strokeColor, size, strokeWidth }`。
...
{"id":"action","component":"Button","label":"打开","onClick":[{"call":"clickToDeeplink","args":{"bundleName":"","abilityName":"","uri":{"path":"/action/uri"}}}]}
...
- `Image.src` 和 `styles.backgroundImage` 只使用本地/资源路径；不支持网络 URL、SVG、base64 SVG 或占位图；没有真实资源时省略 `Image`。
