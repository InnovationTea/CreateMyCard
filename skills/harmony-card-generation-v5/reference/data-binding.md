...
注意：下文“EventHandler 数据”的 `args`/`condition`/`$context` 表达式是事件场景的固有写法，没有原生 `{path}` 等价物，不属于这里说的可被原生绑定替代的兜底。
...
事件参数、`condition` 用表达式读取 DataModel、事件上下文或行为链变量。这是事件场景的固有写法（没有原生 `{path}` 等价物），不是可被原生绑定替代的兜底：
