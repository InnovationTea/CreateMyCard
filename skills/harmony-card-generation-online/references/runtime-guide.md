...
8. 基于合法 overview 选择候选并执行动态数据能力满足度门禁：全部不可用则结束，部分可用则保留可用候选降级。
...
overview 前仅检查卡片形态、静态范围和最小语义歧义。用户 query 明确不属于 Skill 支持的静态形态时立即结束；不得根据 query、历史、缓存或经验裁决动态数据能力是否满足，也不得在此阶段追问数据参数。取得本轮合法 overview 后，才对用户所需数据能力执行可用性裁决：全部无法满足时结束，部分可用时仅保留可用部分降级生成，不得伪造缺失能力。仅在 overview 保留可用候选并加载 schema 后，发现 `inputSchema.required` 中存在用户可回答且会改变核心意图、候选或结果的参数缺失时，才追问最小必要问题并等待；不询问设备支持情况、应用安装情况、权限、能力 ID、schema、写入路径或协议版本。
...
| 决策 | 条件 | 后续 |
| --- | --- | --- |
| 继续生成 | overview 后所需数据能力均满足，或静态/入口卡无需动态数据 | 继续 |
| 结束并引导 | 静态形态不支持，或 overview 后所需数据能力全部无法满足 | 停止后续工具，回复不支持话术并给出相近需求 |
| 调整后生成 | overview 后所需数据能力部分可用 | 删除不可用数据能力，说明差异后按可用内容自动继续 |
| 追问 | overview 后的 schema 显示缺少会改变核心意图、候选或必填参数的用户信息 | 只问最小必要问题并等待 |

用户明确“必须包含，否则不要生成”的能力在 overview 或 schema 阶段不可用时直接结束，不降级；否则只要至少一个所需数据能力可用，即按部分支持降级生成。工具异常不用于推断能力，也不据此推荐。
...
概述或 schema 无法覆盖全部所需数据能力时重新执行满足度决策：全部缺失时结束，部分缺失时从候选中移除不可用能力后继续生成并说明差异；核心动作或必需素材无法满足时也按满足度决策终止。生成前结束或生成返回 `unsupported` 时推荐 1～3 条可复述需求：已有合法概述时优先同领域、低风险且有完整卡片价值的描述；尚无概述时只用天气、日程、运动、设备电量或系统状态等通用示例，并使用“可以试试”，不承诺可用。
...
`[]` 视为空；非字符串数组则 payload 非法。数据候选只能来自 `dataCapabilities`。合法 overview 返回后立即依据 `dataCapabilities` 与 `unavailableCapabilities` 裁决所需动态数据能力：全部不可用则终止，部分不可用则仅保留可用候选并说明降级；不得在此调用前提前裁决。
...
- `XX` 必须替换为用户提及的功能或未满足内容；“建议一”“建议二”必须替换为 Agent 根据本轮能力概述生成的 1～3 条相近需求。两者都不得原样出现在最终回复中。
...
以下话术中的 `XX` 是格式占位符，必须替换为用户原话中对应的功能或未满足内容；“建议一”“建议二”也是格式占位符，由 Agent 基于本轮能力概述生成 1～3 条可直接复述的相近需求，绝不原样输出。已有合法概述时优先同领域、低风险且有完整卡片价值的建议；尚无概述时使用天气、日程、运动、设备电量或系统状态等通用示例，不承诺可用。

- 不支持的 query 或数据能力全部无法满足：`抱歉，当前暂无法获取你提及的XX功能数据。你可以试试：“建议一”、“建议二”`
- 数据能力部分支持：`本次卡片暂无法完整支持你提及的XX，将基于可用内容为你生成卡片` 输出后自动继续，不等待确认；仅当用户明确“必须包含，否则不要生成”时改为终止。
