...
或事件 dynamicArguments 时回填对应值；其它结果和清洗后的会话有效内容写入可选 `extrainfo`，不把完整上下文追加到有效 userQuery。用户可见的 R16/R16W 事实摘要与生成用完整事实载荷分开维护，不能从摘要反推载荷。
只接受结构、类型和业务含义可可靠验证的事实；保留必要单位、日期和地点，不以强制类型转换掩盖非法结果。

### create 条件工具链

- `extrainfo` 是可选字符串数组，仅传本轮已校验、已实际告知、与卡片相关的完整静态事实和会话有效内容；按来源出现顺序保留，空项剔除，仅对完全相同字符串去重，没有内容时省略字段，不传空数组。
- 主 Agent 传入时每个条目必须保留原事实中的日期、时间、地点、数值、单位、条件、否定、限定词、来源归属和语义；允许去除首尾空白，但不得总结、压缩、合并、翻译、截断、重排或改写，也不能只传 R16/R16W 中的短摘要。生成卡片工具内部模型可以在展示层按尺寸和可读性摘要、压缩、合并或改写，但不得捏造、改变事实含义或丢失核心事实。
...
- 生成前逐项检查来源、清洗状态、实际告知状态和完整载荷；任何一项无法追溯或不符合类型时移除，核心事实因此缺失则按主流程停止。普通 A2UI 与 Design Compact 模型收到非空 `extrainfo` 时，必须读取并将相关事实吸收到可见静态内容，可按布局摘要或压缩，但不能只写入内部判断，也不能丢失核心事实。
...
- 超量时依次移除装饰、可选项和次要支撑项，优先换用更简单布局或适当换行；`extrainfo` 入参仍保持完整，生成模型可在展示层摘要或合并，但不得丢失用户明确要求的核心事实。核心事实仍无法表达时按核心/次要缺失规则停止或告知，不提交不完整请求。
...
- 多个来源按相关性串行调用，保持播报和结果顺序；前一个合法结果可用于后一个来源的参数或最终有效 `userQuery`。
