...
| --- | --- | --- | --- | --- |
| C01 | A 动态创建 | O → S → P → R01 → G → R18C | A+B；无 sourceArtifactUrl | 更新为 U1 |
| C02 | 固定文字静态创建 | O → R01 → G → R18C | 数据为空，不调用空 S/P | 更新为 U1 |
| C03 | 仅快捷入口 | O → R01 → G → R18C | 完整复制 E，无数据时不调 P | 更新为 U1 |
| C04 | 天气核心、股票次要且不可用 | O → R08C → S → P → R01 → G → R18C+R19D | query、标题、说明只保留天气 | 更新为 U1 |
| C05 | 股票必须包含但不可用 | O → R07 | 不调用 S/P/G/R01 | 不变 |
| C06 | S 移除最后一个核心数据 | O → S → R07 | missing 不进入候选 | 不变 |
| C07 | 打车核心，仅导航可用 | O → R09 → 等待 | 不调用 S/P/G/R01；确认后仍按 create 重新 O | 不变 |
| C08 | schema 要求的用户偏好缺失 | O → S → R05 → 等待 | 不猜用户偏好、不调 P/G/R01 | 不变 |
| C09 | 卡片上下文中要求长报告 | R06 | 零调用、不先 R01 | 不变 |
| C10 | 普通搜索、名片或网页设计 | 不触发此 Skill | 不借卡片工具执行其它任务 | 不变 |
| C11 | 卡片上下文意图仍有歧义 | R03 → 等待 | 零工具调用、不先 R01 | 不变 |
| E01 | 上一轮动态卡片，本轮“颜色换红色” | P → R02 → G → R18E | 来源 U1，继承数据；无 O/S，不转 create | 更新为 U2 |
| E02 | 静态卡片修改布局/背景 | R02 → G → R18E | 来源 U1，无 P；只传本轮修改 | 更新为 U2 |
| E03 | 标题、文案或尺寸修改 | P（有数据时）→ R02 → G → R18E | 来源 U1；凡用户要求修改且工具有对应参数，G 必须显式传该参数；尺寸修改必须传本轮 `size` | 更新为 U2 |
| E04 | 天气+日历中删除日历 | O → S → P → R02 → G → R18E | 从真实链恢复完整列表，仅保留天气；保留其参数 | 更新为 U2 |
| E05 | 删除全部动态数据改静态文案 | O → R02 → G → R18E | 显式数据 []，不调用空 S/P；仍传 U1 | 更新为 U2 |
| E06 | 上海青浦替换为北京今日天气 | O → S → P → R02 → G → R18E | 同一天气 ID；新城市匹配本轮 schema，删除旧区县 | 更新为 U2 |
...
| E10 | 连续视觉编辑省略数据数组 | P → R02 → G → R18E | 从真实链恢复最终权限集合，用最近有效 U2 | 更新为新 URL |
| E11 | 明确“再做一张/重新创建” | O → S（如有）→ P（如有）→ R01 → G → R18C | 不继承 U1，不含 sourceArtifactUrl | 新卡片节点 |
| E12 | 本轮只说“改一下”且未说明改什么 | R05 → 等待 | 零调用，不擅自 create | 不变 |

## 权限与结果回归矩阵

下表 P 场景均从 A 的 O → S 开始；生成前检查完成后才发送 R01/R02，P 返回前不得进入 X/G。
...
| --- | --- | --- | --- |
| W01 | Skill 前 W0 已提供充分且尚未告知的相关事实，无动态数据 | W0 → 加载 Skill → R16W → O → 拼接 userQuery → R01 → G → R18C | 不重新搜索；事实完整呈现，结果只说明生成与用途 |
| W02 | W0 充分，且此前助手已告知全部采用事实 | W0 → 加载 Skill → O → 拼接 userQuery → R01 → G → R18C | 不重复 R16W，不复述已告知事实；真实来源仍取 W0 |
| W03 | W0 有多条结果，其中部分事实已告知 | 校验采用 → R16W → 必要前置 → 拼接 userQuery → R01 → G → R18C | R16W 仅含尚未告知的完整事实 |
| W04 | W0 只覆盖部分事实，还需补查 | 校验采用 → R16W → 必要前置 → W → R16W → 拼接 userQuery → R01 → G → R18C | 补查仅覆盖不足部分；新事实在生成前实际说明 |
| W05 | 存在无关历史搜索，本轮不采用 | 必要卡片工具 → R01/R02 → G → R18C/R18E | 不把无关搜索当 W0，不凑事实说明；无事实时不追加固定标题 |
...
| W08 | 已有 W0，但本轮权限明确拒绝或正常返回非法 | 必要前置 → P → R11/R12/R13/R14（按结果） | 不采用/回填/说明外部事实，不补查、不生成；按真实结果使用终止话术 |
...
| W17 | 前置来源已将采用事实完整告知，但卡片尚未生成 | R16/R16W → 必要卡片步骤 → 拼接 userQuery → R01/R02 → G → R18C/R18E（尚需结果反馈时） | 事实消息先于开始话术，R01/R02 紧邻生成调用；仍检查真实来源和必要参数 |
| W18 | 补查结果只重复已告知事实，未补齐核心必填值 | 必要前置 → 补查 → 按现有事实补全规则继续或 R17 | 不凑 R16W/R16，不以重复结果掩盖缺失，不提交不完整生成参数 |
| CXT01 | 用户要求把最近一条助手答案做成卡片 | 前文答案 → R16 → 必要卡片步骤 → R01/R02 → G → R18C | 前文答案按非 WebSearch 外部事实处理，步骤 6 将完整事实追加到 userQuery 的固定标题段落 |
| CXT02 | 前文答案含链接、工具字段或推理过程 | 前文答案 → 清洗 → R16 → 必要卡片步骤 → R01/R02 → G | 只保留业务事实，清洗后的完整内容进入 userQuery 固定标题段落，不传链接、内部字段、工具信息或推理过程 |
| CXT03 | 前文答案为空、为失败话术或无法安全提取 | 前文答案 → R05/R14 | 不猜测内容，不调用生成工具 |
| CXT04 | 用户要求将前文答案做成实时刷新卡片 | 前文答案 → R16 → 动态能力判断 → R07/R09 | 静态答案不能冒充动态刷新能力，不能仅靠 userQuery 降级 |
...
| B04 | R16/R16W 的完整事实附加建议、背景知识或获取过程 | 只保留尚未告知的采用事实及必要限定；不夹带额外段落 |
...
| 场景 | 预期顺序 | 约束 |
| --- | --- | --- |
| Skill 前已有外部事实 | 校验采用 → R16/R16W → 动态能力工具（如需）→ 权限（仅有动态集合时）→ 拼接 userQuery 固定标题段落 → R01/R02 → G → R18C/R18E | 外部事实先告知；完整事实拼接只发生在生成前参数构造阶段 |
| Skill 执行中取得非 WebSearch 外部事实 | 必要前置 → 来源返回 → 校验采用 → R16 → G → 结果回复 | 来源可以是 Agent 的其它工具或 Skill；不注册动态能力、不伪造刷新能力 |
| 纯静态外部卡片 | 校验采用 → R16/R16W → 无 S/P → 拼接 userQuery 固定标题段落 → R01 → G → R18C | 外部静态内容可独立满足核心用途 |
| 动态与外部静态混合 | 校验采用 → R16/R16W → O → S → P → 回填参数并拼接 userQuery 固定标题段落 → R01 → G → R18C/R18E | 权限只约束动态集合，事实完整保留在 userQuery |
...
| 场景 | 必须顺序 | 判定 |
| --- | --- | --- |
| Skill 前已有可采用外部事实 | 外部结果 → 校验采用 → R16/R16W → 卡片 SOP → R01/R02 → 生成 | 事实消息实际发送后，开始话术紧邻生成调用 |
| 外部事实尚未采用 | 外部结果 → 卡片任务识别 → 卡片 SOP → R01/R02 → 生成 | 不发送事实话术，不把来源结果写入参数 |
