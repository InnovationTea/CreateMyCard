...
| CXT01 | 用户要求把最近一条助手答案做成卡片 | 前文答案 → R16 → R01 → O/S/P（按数据）→ 步骤 7 回填 userQuery → G → R18C | 前文答案按非 WebSearch 外部事实处理，入口只回复，步骤 7 才回填 |
| CXT02 | 前文答案含链接、工具字段或推理过程 | 前文答案 → 清洗 → R16 → R01 → 必要卡片步骤 → G | 只保留业务事实，不传链接、内部字段、工具信息或推理过程 |
| CXT03 | 前文答案为空、为失败话术或无法安全提取 | 前文答案 → R05/R14 | 不猜测内容，不调用生成工具 |
| CXT04 | 用户要求将前文答案做成实时刷新卡片 | 前文答案 → R16 → R01 → 动态能力判断 → R07/R09 | 静态答案不能冒充动态刷新能力 |
