...
11. **执行权限门禁**：数据集合非空时调用 `RequestDataPermission` 并等待正常结果或明确的 invoke 异常。正常返回时只有契约明确通过才进入下一步，明确拒绝或正常返回结果非法时结束；工具不可用、invoke 抛错、超时、传输失败或工具层执行失败且没有正常权限结果时，按权限默认开启直接进入下一步，不重试、不伪造权限结果、不改变数据集合，也不输出权限异常说明。集合为空时跳过；权限检查后数据集合变化时重新检查。
...
所有箭头都以当前步骤结果合法且门禁通过为前提；除权限工具 invoke 级异常按默认开启继续外，任一步失败立即终止，不继续调用后续工具。
