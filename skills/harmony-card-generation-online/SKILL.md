...
6. 微服务工具向主 Agent 展示的内容就是本次完整结果，不得继续寻找未展示的外层或其它返回通道。
   结果为对象时直接读取字段；结果为文本时只从当前完整文本中识别明确呈现的业务字段，无法可靠识别
   即按结果非法终止。生成工具返回后，先从当前结果明确呈现的 `artifactUrl` 锁存合法真实 URL，再判断
   状态和话术；历史回复或无字段语义的普通文本 URL 不算产物 URL。
7. 只要当前生成结果含合法真实 `artifactUrl`，同一条最终回复就必须且只能输出一个以下代码块；没有
   URL 时不得输出，edit 返回来源 URL 时也按无有效新 URL 处理：
