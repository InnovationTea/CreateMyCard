...
### 0. 处理 Skill 前已取得的外部事实

- **进入条件：** 当前任务真实工具轨迹中存在 WebSearch、其它外部工具或 Skill 返回，且用户明确提出了卡片创建、预览或连续编辑意图。
- **用户回复：** 先校验并确定本次卡片实际采用的外部事实，再实际发送 R16W（WebSearch）或 R16（其它来源）。事实摘要直接呈现，不描述搜索、调用、校验或准备过程。
  该消息必须先于 R01/R02；内部草稿、工具返回和写入 `userQuery` 均不算已回复。没有采用的外部事实不发送。
- **工具调用示例：** 本步不调用本 Skill 固定工具。外部结果来自当前任务真实轨迹，按运行指南校验后再回填。
- **参数来源与返回检查：** 可匹配本轮 `inputSchema` / `dynamicArguments` 的事实仅补充已有业务参数，其它事实追加到有效 `userQuery`；不得新增动态能力、事件、素材或伪造刷新能力。
...
- **来源与检查：** 同时识别当前任务真实工具轨迹中已有的外部来源结果（包括 WebSearch、其它外部工具或 Skill），允许搜索发生在 Skill 加载之前；
  本步只识别来源与任务关联；步骤 0 已完成前置外部事实的采用、回填和播报。结合当前需求和真实工具轨迹判断模式；
...
- **进入条件：** 步骤 0 已识别并处理当前任务既有外部来源，或本次补查已返回；
...
- **用户回复——已有 WebSearch：** 对步骤 0 已取得的多条结果，汇总本轮采用事实并在卡片 SOP 前发送 R16W；
  步骤 0 未完成实际发送时，不得继续后续卡片工具。
...
- **工具调用示例：** 本步零调用。模拟本任务在 Skill 前通过 WebSearch 获取演出时间和地点；
  校验采用后先实际发送 R16W，再发送 R01/R02 并继续卡片 SOP。
