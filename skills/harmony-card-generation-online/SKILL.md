...
本节是用户可见回复的唯一调度入口，优先于流程中的其它叙述。凡流程节点需要播报、追问、说明结果或结束引导，必须先回到本节确定回复状态，再按已读取的 [`references/runtime-guide.md`](references/runtime-guide.md) 中“用户回复规范（唯一出口）”章节使用对应话术；不得凭普通对话习惯自行改写、拼接工具结果或继续调用。

| 回复状态 | 必须发送的内容 | 发送后动作 |
| --- | --- | --- |
| 首个工具调用前且已确认继续调用 | create：`好的，我现在为你创建卡片。`；edit：`好的，我现在按你的要求修改卡片。` | 只发送一次，继续工具链 |
| 仍缺少会改变核心结果的信息 | 只追问一个最小必要问题 | 等待用户，不调用下一工具 |
| 核心能力不足、明确不支持或替代待确认 | 使用运行指南对应的结束/替代话术 | 结束本轮或等待确认，不生成 |
| 次要内容不可用但核心目标仍成立 | 先说明将移除该内容 | 不等待确认，继续生成；后续需求不得保留已移除内容 |
| 外部来源调用中 | 不向用户发送调用前播报 | 静默调用该来源 |
| 外部来源成功取得所需数据后 | **必须立即发送一条结果播报**：`已调用「{显示名}」获取到{数据内容}` | 先完成数据校验并发送该播报，再调用后续来源或生成；不得把来源工具/Skill 返回当作用户回复 |
| 生成成功且返回合法新 URL | **必须立即发送一条最终用户回复**，明确说明卡片用途并总结卡片内容；降级时另说明确定未包含内容 | 先发送该回复，再结束本轮；不得把工具返回当作用户回复 |
| 生成失败、结果非法或 unsupported | 使用运行指南对应的失败/引导话术 | 结束本轮，不伪造结果 |

每次发送前执行检查：只发送当前状态允许的一条回复；不输出 `artifactUrl`、Markdown 链接、工具包络、能力 ID、schema、provider、错误码、DSL、CardSpec 或未确认的具体字段/动作。运行时发现的外部来源工具或 Skill 调用、以及生成工具调用都是中间步骤，工具返回内容不会自动成为用户回复。外部来源成功并完成数据校验后，**不得直接调用下一个来源或生成工具、不得只保留工具结果、不得省略结果播报**，必须先发送 `已调用「{显示名}」获取到{数据内容}`；收到生成 `success/degraded` 且 URL 合法后，**本轮不得直接结束、不得只返回工具结果、不得省略自然语言**，必须先按运行指南“生成后自然语言”发送一条非空最终回复。生成成功回复必须同时包含“用途”和“内容总结”，不得声称已添加到桌面、已安装或已替用户开启权限。
