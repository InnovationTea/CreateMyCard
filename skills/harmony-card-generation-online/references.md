...
此 skill 是云侧工具编排版，只负责 create/edit 模式判断、需求适配、生成前能力满足度判断、候选选择、来源 URL 传递、工具调用和用户回复组织。卡片产物必须由 `generateWidgetCard` 生成；生成前合法结束不调用生成接口，任一必要工具不可用、调用失败或结果不完整时终止本轮生成或编辑。

本版本只实现云侧工具编排。旧版 `harmony-card-generation`、历史模板和离线资料不能作为生产候选或产物依据。

## 按场景读取

- 所有请求先读取 [`references/orchestration-workflow.md`](references/orchestration-workflow.md)，按职责边界和完整十二步流程推进。
- 需要追问、生成前说明、结束并引导或处理工具结果时读取 [`references/response-policy.md`](references/response-policy.md)。
