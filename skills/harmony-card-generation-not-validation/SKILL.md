...
写 DSL 前必须且只能选择一个尺寸。尺寸选择按“先压缩到 `2x2`，再证明必须 `2x4`”的顺序：

1. 先筛选到一个主答案、最多一个短上下文组和最多一个主动作。
2. 如果这些内容能通过竖向堆叠、单个视觉锚点、小图标/字形、短 CTA 或一个紧凑信息块放入 `2x2`，必须选择 `2x2`。
3. 只有能明确说明 `2x2` 的失败点时，才升级到 `2x4`。失败点必须是受保护文本、热区、独立 pane 数量或必须并列的非装饰媒体放不下，而不是“看起来更丰富”。
...
- 睡眠状态、会议提醒、关怀/联系、倒计时 + 单条计划、耳机/低电/连接状态、防沉迷时长等单主题卡片，默认都应先按 `2x2` 处理。
- 有图片或产品主体不自动升级；如果图片只是一个视觉锚点，并且只配 1 到 2 个状态/指标和一个动作，仍然使用 `2x2`。
- 主区域：<= 3。
- 适合无需滚动、无需阅读段落即可一眼理解的答案。

### `2x4` / `160vp（高）x 320vp（宽）`

仅当 `2x2` 不能无损承载核心信息时使用。选择 `2x4` 前必须写出 `2x2` 不够的具体原因，并至少满足一个硬条件：

- 有 2 到 3 个必须并列理解的独立 pane，例如主指标 + 多行详情 + 动作，且竖向堆叠会挤压受保护文本或热区。
- 受保护标题、时间、状态、CTA、价格/数量等在 `2x2` 中经过字段筛选、减 padding/gap、降字号、拆行后仍无法完整显示。
- 非装饰媒体必须与状态/详情/动作同时可见，并且媒体本身承载关键信息；若媒体只是装饰或单个视觉锚点，不构成升级理由。
- 用户明确要求横版、并排对比、双面板、左右关系或 3 个以上紧凑辅助事实。
- 主区域：<= 4。
- 默认不滚动。如果卡片需要滚动，它很可能已经接近页面。

不要仅因为出现图片、设备/产品、详情 + 动作、两个短字段、按钮、漂亮留白或历史示例是横版，就选择 `2x4`。
...
- 出现图标、图片、背景图、媒体路径，或会议/时间/身份类视觉锚点：先读 [`reference/asset-library.md`](reference/asset-library.md) 按语义匹配已声明素材；若匹配到素材且仍需要该语义视觉锚点，必须使用匹配素材的 `src`，不要改用自造图标、文本字形或相似路径；若无匹配素材，再按 [`reference/visual-interaction.md`](reference/visual-interaction.md) 或 [`reference/expressiveness-toolkit.md`](reference/expressiveness-toolkit.md) 选择非媒体方案。
...
6. 如果使用图标、图片或视觉素材，先从素材库选择语义匹配的已声明 `src`；一旦匹配到素材且该语义视觉锚点仍出现在卡片中，必须使用该 `src`，不得用 `Text` 字形、emoji、SVG、自绘形状、相似资源路径或未声明图片替代。静态素材可直接写入 `Image.src`，需要数据驱动选择时绑定到 `/asset/...` 并在 DataModel 初始化为已声明 `src`。
...
- 目标卡片尺寸为 `2x2 = 160 x 160vp` 和横版 `2x4 = 160vp（高）x 320vp（宽）`（DSL 中写作 `width: 320`、`height: 160`）。如果宿主要求 `width: "100%"`，仍然按所选宽度预算设计：`2x2` 为 `160vp`，`2x4` 为 `320vp`。
- 内容层距离卡片四周至少 `12vp`；背景、渐变、图片或光效可贴边，但主标题、主数字、状态、CTA 和可点击控件必须避开圆角裁剪区。
- 外层圆角预览：`2x2` 使用 `18vp`，`2x4` 使用 `22vp`。
...
- 可点击元素可见尺寸不得小于 `24vp`，热区不得小于 `32vp`；按钮使用圆形或胶囊形态，按钮文本保持一行完整显示。
...
- 不要编造远程媒体 URL 或未声明资源路径。`Image.src` 和 `styles.backgroundImage` 只使用用户提供或素材库声明的本地/资源路径；不支持网络图片或 SVG。素材库语义匹配成功时，不要跳过素材库去生成自造图标、emoji、文本字形或相似图标。
...
- 关键信息默认必须完整显示：日期、星期、时间、CTA、状态、主指标/数值、主标题/名称、倒计时、价格/数量，以及字段筛选后决定展示的用户字段。若用户需求涉及太多数据字段，先按 `capability.md` 的字段筛选保留最多 4 项；未入选字段不作为受保护内容，也不强行塞入卡片。不要依赖 `textOverflow: "ellipsis"`、`clip` 或 marquee 隐藏已选关键信息。
