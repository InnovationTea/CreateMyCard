...
{"version":"v0.9","updateComponents":{"surfaceId":"sample-card","components":[{"id":"root","component":"Column","children":["title"],"styles":{"width":160,"height":160,"padding":12,"borderRadius":18,"clip":true}},{"id":"title","component":"Text","content":{"path":"/title"},"styles":{"fontSize":16,"fontWeight":700}}]}}
...
```json
{"id":"root","component":"Column","children":["primary"],"styles":{"width":160,"height":160,"padding":12,"borderRadius":18,"clip":true}}
```

```json
{"id":"root","component":"Row","children":["focal","details","status"],"styles":{"width":320,"height":160,"padding":12,"borderRadius":22,"clip":true}}
```
...
- `2x2`：root `width: 160`，`height: 160`，内容安全边距 `12`，圆角预览 `18`，2 到 3 个主区域。
- `2x4`：root `width: 320`，`height: 160`（即 `160vp（高）x 320vp（宽）`），内容安全边距 `12`，圆角预览 `22`，2 到 4 个主区域横向排列。
- 默认先尝试 `2x2`。只有当受保护文本、按钮热区、必须并列 pane 或关键媒体在 `2x2` 中放不下时，才选择 `2x4`。
- 不要因为出现图片、产品、详情、动作或两个短字段而自动选择 `2x4`；这些通常可以做成 `2x2` 的单个视觉锚点 + 短上下文。
- 背景、渐变、图片或光效可以贴边；主标题、主数字、状态和 CTA 保持在 `12vp` 安全区内。
...
- 字号避免页面化：标题常用 `14fp`/`18fp`，主数字常用 `32fp`/`40fp`，辅助信息常用 `12fp`/`14fp`，提示性文本最小 `10fp`。
- 对受约束的标题值，优先使用 `maxLines: 1` 和 `minFontSize`。
- 不要在日期、星期、时间、状态、CTA、主指标、主标题或字段筛选后决定展示的用户字段等关键信息上使用 `textOverflow: "ellipsis"`。如果用户需求涉及太多数据字段，先保留最多 4 项再保护完整显示；通过减少 padding/gaps/字号、移动次要文本、拆行或选择 `2x4` 让已选关键文本放下。
...
- Form 只支持 `onClick`。
- 默认最多一个主动作。
- 可点击元素可见尺寸不得小于 `24vp`，热区不得小于 `32vp`。
- `2x2` 圆形按钮推荐 `30 x 30vp`；胶囊按钮推荐高度 `36vp`、文本 `14fp Medium`。
...
- 需要图标、图片或视觉锚点时，先按 [`asset-library.md`](asset-library.md) 语义匹配已声明素材；匹配成功且该语义视觉锚点仍展示时，必须使用匹配素材的 `src`。用户明确指定的本地/资源路径也可使用。
...
- 不要用 `Text` 字形、emoji、自绘形状或相似图标替代已匹配素材。
...
- [ ] Root 适配 `160 x 160vp` 或横版 `160vp（高）x 320vp（宽）`。
