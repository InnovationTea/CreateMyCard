...
- 素材库存在明确语义匹配，且卡片仍需要该语义视觉锚点，却没有使用匹配素材的 `src`。
- 用 `Text` 字形、emoji、自绘形状、相似图标或未声明路径替代已匹配素材库素材。
...
- 未明确选择 `2x2` 或 `2x4`。
- 在 `2x2` 可容纳一个主答案、短上下文和一个动作时选择了 `2x4`。
- 选择 `2x4` 但没有说明 `2x2` 的具体失败点：受保护文本、按钮热区、必须并列 pane 或关键媒体放不下。
- `2x2` 主区域超过 3 个，或 `2x4` 主区域超过 4 个。
- 内容层没有 `12vp` 安全区，或主标题、主数字、状态、CTA、可点击控件进入圆角裁剪风险区。
- `2x2` 外层圆角预览不是 `18vp`，或 `2x4` 外层圆角预览不是 `22vp`。
- 主要信息需要滚动、长列表、表格、段落或页面导航才能理解。
- 卡片由示例/模板改造而来，而不是由语义角色和构图规则生成。
- 正式输出前没有布局理由或没有至少一次显式改进。
- 可点击视觉区域没有 `onClick` EventHandler。
- 可点击元素可见尺寸小于 `24vp`，或可点击热区小于 `32vp`。
...
1. 列出该行中的受保护内容：日期、星期、CTA、时间、状态、百分比、价格、短标签、主数字、主标题和字段筛选后决定展示的用户字段；当用户需求涉及太多数据字段时，只评审入选的最多 4 项用户字段。
2. 检查它是否位于狭窄固定宽容器中。
